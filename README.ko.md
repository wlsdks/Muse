...
나를 학습하고, 내가 필요로 할 것을 미리 알아보고, 나를 대신해
처리해 주는 개인용 AI 비서. 먼저 말 걸어야 할 때 먼저 말하고,
물으면 즉시 답한다. 내 컴퓨터에서, 내가 고른 모델로, 데이터는
로컬에 둔 채.
...
Muse는 내가 실제로 위임할 수 있는 비서를 지향한다. 실제 사용에서
나에 대한 그림을 만들고, 묻기 전에 내가 원할 만한 것을 알아채고,
직접 알아본 뒤, 처리하거나 먼저 확인한다. 채팅·음성·터미널 등
내가 있는 곳으로 닿고, 캘린더·노트·태스크·메시징·웹·내 컴퓨터 등
실제 도구로 행동한다.

그리고 그 전부를 내가 소유한다. 어떤 LLM이든(로컬 Ollama/Qwen
또는 모든 클라우드), 어떤 도구든, 어떤 MCP 서버든 — 동일 런타임이
CLI·API·웹 UI를 구동하므로 벤더나 빌드에 묶이지 않고, 데이터는
기본적으로 로컬에 남는다. 내부 구조:

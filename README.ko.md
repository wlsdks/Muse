...
Muse는 **전부 네 기기 안에서** 돌아가며 **네 메모와 파일에서** 답하는 개인 AI
컴패니언이다 — 출처까지 인용하고, 모르면 "잘 모르겠어"라고 말하고, 자기 약점을
찾아 고치며 실제로 더 똑똑해진다. 한 글자도 밖으로 안 나간다 — 약속이 아니라
코드로 막아뒀다.
...
1. **로컬 by construction — _그래서 다 말해도 된다._**
   전부 로컬 오픈소스 모델(Ollama qwen3:8b)에서 돌고, 클라우드 유출은 **코드로
   거부**된다(`MUSE_LOCAL_ONLY` 기본 ON). 남의 클라우드 위 에이전트가 아니라 진짜 네 것.

2. **근거를 보여준다 — _절대 지어내지 않는다._**
   모든 답·알림·통찰이 실제 출처를 인용하고, 근거 약하면 "잘 모르겠어", 근거 없는
   주장은 **코드가 드롭**한다. fabrication=0이 릴리스 게이트(회상·proactivity·반성·chat 전부).

3. **Whetstone(숫돌) — _자기 약점을 고쳐 더 강해진다._**
   Muse는 자기가 상습적으로 틀리는 것(하지 말아야 할 거절, 안 했는데 했다고 한 것)을
   알아차려 기록하고 체계적으로 갈아낸다 — 고정된 뇌로 발전하는 사람처럼, 가중치 변경
   없이. 평범한 모델도 날을 세우면 더 잘 벤다. (`muse doctor --weaknesses`)
...
원칙 1이 *왜* 다 말할 수 있는지, 원칙 2–3이 *그래서 무엇을 해주는지*, 원칙 4가
*어떻게* 계속 베낄 수 없는 능력을 얻는지. 깊이: [엣지](docs/strategy/the-edge.md) ·
[Whetstone](docs/strategy/whetstone.md) ·
[교차분야 카탈로그](docs/strategy/cross-field-research.md).

...
벤더 중립의 JARVIS형 개인용 AI 컨덕터. 어떤 LLM이든, 어떤 도구든,
어떤 MCP 서버든 하나의 일관된 추론 루프로 오케스트레이션한다.
...
동일한 `agent-core` 런타임이 API 서버, CLI, React 웹 UI를 모두
구동하며 — 모델 공급자는 빌드 시점이 아닌 런타임에 선택한다.
...
packages/
  ...
  model/              ModelProvider 인터페이스와 공급자 wire-format
                      어댑터 (OpenAI / Anthropic / Gemini / Ollama)
...
| `MUSE_USER_MEMORY_AUTO_EXTRACT` | `false` | 매 턴 후 LLM이 사실/선호 자동 추출 |

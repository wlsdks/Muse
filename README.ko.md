...
고른다. 자동으로 알아채는 기능은 그다음이다. 이 경험은 아직 개발 목표이며 출시된 기능은
아니다. **업무 복귀(Work Resumption)**는 그중 업무에 특화된 사용법이지 Muse 전체가 아니다.

| 지금 제공하는 토대 | 앞으로 연결할 것 |
| --- | --- |
| 개인 기억, 출처가 보이는 회상, 로컬 개인 저장소, 사용자가 켜는 주변 맥락, 알림 예산, 승인형 브라우저 행동, 실행 기록 | 삶의 맥락 묶음 → 사용 결과 → 알맞은 타이밍 → 다음 도움 개선 |

...
describe("Personal Continuity store", () => {
  ...
  it("reads schema v5 byte-stably, migrates on explicit mutation, and rejects v5 contact laundering", async () => {
    ...
    const legacy = current.replace('"schemaVersion": 9', '"schemaVersion": 5');
    writeFileSync(file, legacy, "utf8");

    expect((await readAttunementState(file)).schemaVersion).toBe(9);
    ...
    const forgedLegacy = readFileSync(file, "utf8").replace('"schemaVersion": 9', '"schemaVersion": 5');

...
describe("Attunement schema v9 browsing-visit references", () => {
  it("reads v8 byte-stably in memory and writes v9 only on explicit mutation", async () => {
    const file = await storeFile();
    const raw = `${JSON.stringify(state(8), null, 2)}\n`;
    await writeFile(file, raw, "utf8");

    expect(await readAttunementState(file)).toMatchObject({ schemaVersion: 9 });
    expect(await readFile(file, "utf8")).toBe(raw);

    await createPersonalThread(file, { kind: "work", title: "Ship exact browsing context" });
    expect(JSON.parse(await readFile(file, "utf8"))).toMatchObject({ schemaVersion: 9 });
  });

...
describe("applyEdit / applyEdits (pure, no disk)", () => {
  ...
  describe("fuzzy fallback (Codex-style, exact-first)", () => {
    ...
    describe("indentation preservation on a fuzzy (trim-relaxed) match", () => {
      it("re-bases a multi-line new_string onto the FILE's indentation, not the model's guess", () => {
        // File block is tab-indented (2 tabs); the model recalled old_string at
        // 2-space indent — a tab/space mismatch can never be an exact substring
        // match, so this is genuinely forced through the trim() relaxation. The
        // spliced result must carry the FILE's tab indent, and the blank line in
        // new_string must stay blank (not indented).
        ...
        expect(lines).toContain("\t\tdoThing();");
        expect(lines).toContain("\t\tlog2();");
        expect(lines.some((line) => line === "log2();")).toBe(false);
        // The blank line in new_string stayed blank, not indented.
        const doThingIdx = lines.indexOf("\t\tdoThing();");
        expect(lines[doThingIdx + 1]).toBe("");
        expect(out.content).not.toContain("\t\tlog();");
      });

      it("an exact match (indentation included) is byte-identical — no spurious re-indent", () => {
        const file = "  const x = 1;\n";
        const out = applyEdit(file, { new_string: "  const x = 2;", old_string: "  const x = 1;" });
        expect(out).toEqual({ content: "  const x = 2;\n", ok: true });
        expect((out as { fuzzy?: boolean }).fuzzy).toBeUndefined();
      });

      it("a trailing-whitespace-only fuzzy match (indentation already equal) is a no-op re-indent", () => {
        const file = "  alpha\n  beta\n";
        const out = applyEdit(file, { new_string: "  ALPHA", old_string: "  alpha   " });
        expect(out).toMatchObject({ content: "  ALPHA\n  beta\n", fuzzy: true, ok: true });
      });
    });

    describe("escape-drift extension (\\\" / \\' / \\\\)", () => {
      it("repairs a model that double-escaped a quote as literal \\\" where the file has a bare quote", () => {
        const file = 'const msg = "hello";\n';
        const out = applyEdit(file, { new_string: 'const msg = \\"bye\\";', old_string: 'const msg = \\"hello\\";' });
        expect(out).toMatchObject({ fuzzy: true, ok: true });
        if (out.ok) {
          expect(out.content).toBe('const msg = "bye";\n');
        }
      });

      it("still fails closed when the quote-repaired form ALSO doesn't match (no guessing)", () => {
        const file = 'const msg = "hello";\n';
        const out = applyEdit(file, { new_string: 'const other = \\"bye\\";', old_string: 'const other = \\"hello\\";' });
        expect(out.ok).toBe(false);
      });
    });

    it("a genuinely absent old_string still fails closed with the existing hint", () => {
      const out = applyEdit("abc\n", { new_string: "z", old_string: "totally missing line" });
      expect(out.ok).toBe(false);
      if (!out.ok) {
        expect(out.reason).toContain("file_read");
      }
    });

    it("an ambiguous fuzzy match (multiple blocks) is still refused", () => {
      const file = "  return 1;\n  return 1;\n";
      const out = applyEdit(file, { new_string: "return 2;", old_string: "return 1;" });
      expect(out.ok).toBe(false);
      if (!out.ok) {

 ...
 */
/** Auto-extracted typed slots are INFERENCES, not user assertions — they
 * carry a moderate confidence so they decay on the standard half-life,
 * fall out of the prompt snapshot when stale (floor 0.2), and surface on
 * the reconfirm card instead of masquerading as asserted-forever facts.
 * User-stated slots (CLI `user model add`, confirmed cards) stay
 * confidence-free = asserted. */
export const AUTO_EXTRACT_SLOT_CONFIDENCE = 0.6;
  ...
  if (upsertSlot) {
    ...
    for (const slot of vetoSlots) {
      const veto: UserVetoSlot = {
        confidence: AUTO_EXTRACT_SLOT_CONFIDENCE,
        id: slot.id,
        kind: "veto",
        updatedAt: now,
        value: slot.value,

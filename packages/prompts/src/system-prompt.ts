/**
 * The system-prompt builder core. This module is a LEAF: it imports only
 * from other leaves (identity-core, prompt-text, exemplar-retriever) and
 * never from `./index.js`. `compose.ts` depends on the builders here, so
 * anything in this file that reached back through the barrel would make
 * `index.ts` and `compose.ts` a runtime import cycle — which is exactly
 * what this split removed.
 */

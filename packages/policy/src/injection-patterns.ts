...
export const sharedInjectionPatterns: readonly InjectionPattern[] = [
  ...
  // the browser") tripped this and the input guard blocked the whole turn. A
  // real SSRF-instruction injection carries the fetch verb; a URL alone does not.
  { name: "command_injection", regex: /(curl|wget|fetch).{0,10}(internal|localhost|127\.0\.0|10\.|172\.|192\.168)/is },

...
describe("buildUserModelcomposer — opt-in MUSE_RICH_USER_MODEL gate", () => {
  it("default OFF ⇒ no composer (byte-identical default section path)", () => {
    expect(buildUserModelComposer({})).toBeUndefined();
    expect(buildUserModelComposer({ MUSE_RICH_USER_MODEL: "false" })).toBeUndefined();
  });

  it("MUSE_RICH_USER_MODEL=true ⇒ supplies a composer that emits the recall learned block", () => {

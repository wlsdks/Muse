/**
 * Diagnostic-provider deterministic-output helpers. Used only by the
 * diagnostic provider (adapter-diagnostic.ts); kept out of provider-wire so
 * that barrel stays a pure re-export. Re-exported from provider-wire.ts so
 * historical `./provider-wire.js` consumers are unchanged.
 */

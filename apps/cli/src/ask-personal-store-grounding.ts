...
import { buildCalendarContextBlock, buildContactContextBlock, buildReminderContextBlock, buildTaskContextBlock, contactMatchScore } from "@muse/recall";
...
export async function buildPersonalStoreGrounding(params: {
  ...
  const { query, tasks, calendar, calendarDays, reminders, contacts } = params;
  ...
  if (calendar) {
    ...
    try {
      ...
      for (const provider of registry.list()) {
        try {
          collected.push(...(await provider.listEvents({ from, to })));
        } catch {
  ...
  if (reminders) {
    try {
      const all = await readReminders(resolveRemindersFile(process.env as Record<string, string | undefined>));

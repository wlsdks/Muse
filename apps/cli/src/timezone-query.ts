...
registerZone("America/Los_Angeles", "Los Angeles", "pst", "pdt", "pt", "los angeles", "la", "san francisco", "seattle", "pacific");
registerZone("America/Denver", "Denver", "mst", "mdt", "mt", "denver", "mountain");
registerZone("America/Chicago", "Chicago", "cst", "cdt", "ct", "chicago", "central", "dallas", "austin");
registerZone("America/New_York", "New York", "est", "edt", "et", "new york", "nyc", "boston", "eastern", "miami", "toronto");
registerZone("Europe/London", "London", "bst", "london", "uk");
registerZone("Europe/Paris", "Paris", "cet", "cest", "paris", "berlin", "madrid", "rome", "amsterdam");
registerZone("Asia/Kolkata", "India", "ist", "india", "mumbai", "delhi", "bangalore", "bengaluru");
registerZone("Asia/Tokyo", "Tokyo", "jst", "tokyo", "japan", "osaka");
registerZone("Asia/Seoul", "Seoul", "kst", "seoul", "korea");
registerZone("Asia/Shanghai", "China", "shanghai", "beijing", "china");
registerZone("Asia/Hong_Kong", "Hong Kong", "hong kong", "hk");
registerZone("Asia/Singapore", "Singapore", "singapore", "sgt");
registerZone("Asia/Dubai", "Dubai", "dubai", "uae", "gst");
registerZone("Australia/Sydney", "Sydney", "sydney", "aest", "aedt", "melbourne");
...
export type TimezoneQuery =
  | { readonly kind: "convert"; readonly minutes: number; readonly from: Zone; readonly to: Zone }
 ...
 * "<time> <zone> in/to <zone>" (convert) and "what time is it in <zone>" (now).
 * Returns null unless every named zone resolves AND (for convert) the time
 * parses, so a non-timezone question never short-circuits recall.
 */
export function detectTimezoneQuery(query: string): TimezoneQuery | null {
  ...
  if (conv) {
    ...
    if (minutes !== null && from && to) {
      return { kind: "convert", minutes, from, to };
    }
  }
  return null;
}
...
export function formatTimezone(q: TimezoneQuery, now: Date): string {
  if (q.kind === "now") {
    ...
    return `It's ${formatClock(local.getUTCHours() * 60 + local.getUTCMinutes())} in ${here.label} right now.`;
  }

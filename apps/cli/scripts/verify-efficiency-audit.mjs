...
// OLD relevance gate (raw substring) — replicated to compare against the new
// word-boundary gate that the live planForContext now uses.
function oldSubstringExposed(prompt) {
  const p = prompt.toLowerCase();
  return tools.filter((t) => {
    const kw = t.definition.keywords ?? [];
    return kw.length === 0 || p.trim().length === 0 || kw.some((k) => p.includes(k.toLowerCase()));
  });
}
...
console.log(`\n=== Slice 26: tool relevance (new word-boundary planForContext vs old substring vs unfiltered) ===`);
for (const prompt of ["what's the weather in Busan?", "research the housing market", "lock the front door and turn off the lights", "remember my dentist is Dr. Kim"]) {
  const plan = createWorkspaceToolRoutingPlan(tools, { prompt, localMode: true });
  const newN = plan.exposedToolNames.length;
  const oldN = oldSubstringExposed(prompt).length;
  const newTokens = plan.tools.reduce((s, t) => s + tok(JSON.stringify(t.definition.inputSchema ?? {}) + (t.definition.description ?? "")), 0);
  console.log(`  "${prompt}"\n     exposed: unfiltered ${tools.length} → old-substring ${oldN} → new ${newN}  (~${newTokens} tok vs ~${allToolTokens} unfiltered; distractors removed vs old: ${oldN - newN})`);
}
...
console.log(`\n=== Verdict ===`);
console.log(`Slice 26 NECESSARY: ${tools.length} tools registered — dumping all ≈ ${allToolTokens} tok/turn; relevance filtering is essential, not cosmetic.`);
console.log(`Slice 27 scales with skill count (0 saved at 0-1 skills; ~75% at 4+ skills).`);
console.log(`Slice 28 is a long-session guardrail (0 saved under 40 msgs; grows unbounded without it).`);

...
- **일정**: 여러 캘린더(구글·CalDAV·맥 기본·로컬) 연동, 자연어로 조회·추가·수정·삭제, 빈 시간 찾기, 표준 파일(.ics)로 내보내기.
- **할일**: 자연어 마감일 이해, 긴급 표시, 마감 임박 조회, 태그로 걸러보기.
- **알림**: 지정 시각 알림, 반복 알림, 스누즈. 아침 브리핑에도 함께 뜸.
- **연락처**: 이름·연락처·별칭·생일 관리, 주소록 일괄 가져오기, 다가오는 생일 미리 알림, **수신자가 모호하면 추측 않고 되묻기.**
...
- **출처 인용**: 답에는 "이건 어디서 나온 내용"이라는 **출처 표시**가 붙어, 지어낸 게 아니라 내 데이터에 근거한 답임을 알 수 있습니다.
...
- **감시(웹·집·파일)**: 특정 웹페이지/스마트홈 기기/로컬 파일을 지켜보다 "○○ 글자가 나타나면/사라지면/숫자가 기준 이하로 떨어지면" 같은 **조건이 충족될 때만** 알려줍니다.
...
- **능동 알림**: 임박한 할일·일정·알림을 지정한 메신저로 **먼저** 보냅니다(중복 발송 안 함).
...
- **상시 목표**: "○○ 조건이 되면 △△ 해줘" 같은 지속 목표를 등록하면 조건 충족 시 자동 행동합니다 — 단, 밖으로 나가는 행동은 사전 동의가 있어야만.
...
- **스킬 자작·정돈**: 대화 중 받은 교정에서 절차형 "스킬"을 세션 끝에 스스로 써두고, 다음에 비슷한 요청에 활용합니다. 오래 안 쓴 스킬은 보관하고, 비슷한 스킬이 쌓이면 하나로 합칩니다. 자동 작성 스킬은 실행 권한이 없고 위험 패턴 검사를 거쳐, 오염된 교정이 굳는 걸 막습니다.
- **학습 전략 정돈**: 교정에서 배운 작업 전략들이 비슷하게 쌓이면 중복을 하나의 일반 전략으로 합칩니다.
- **선호 자동 추론**: 세션이 끝날 때 그 세션의 교정에서 안정적인 선호를 스스로 추론해 나에 대한 모델에 반영합니다(날조하지 않음).
...
- **안전 규칙(코드로 강제, 부탁이 아님)**:
  1. 자동 전송 절대 금지 — 사람이 그 내용 그대로 확인해야 나감.
  2. 승인 게이트는 막힘 우선 — 확인 못 받으면 안 함.
  3. 수신자는 확정만, 추측 금지.
  4. 모든 전송(보냄/거부 모두) 기록 + 되돌리기 대상.
  5. **은행·결제·송금은 영구히 기능 범위 밖** — 연결하지 않음.
- **결제 동사 제외**: 웹 행동에서도 구매·주문·결제(buy/order/checkout)는 의도적으로 빠져 있습니다.

## 11. 음성 & 멀티에이전트

- **음성 대화**: 눌러서 말하면 음성을 글로 바꿔 전달하고 답을 음성으로 읽어줍니다. 음성 인식·합성은 로컬 엔진 또는 클라우드 중 선택(로컬-온리에선 로컬만). 호출어로 깨우는 모드도 있습니다.
- **여러 에이전트 협업**: 전문 에이전트들을 **순차·병렬·경쟁(가장 먼저 끝난 것 채택)**으로 돌립니다.
- **모델 티어링**: 한 작업 안에서 간단한 조회는 빠른 모델에, 깊은 추론은 강한 모델에 자동 배분합니다.
- **에이전트 명세**: 역할·도구·지시문을 가진 에이전트를 등록하고, 요청에 맞는 것을 골라 씁니다.

## 12. 신뢰·관측·사용 표면

- **입력/출력 방어**: 숨은 명령 주입(프롬프트 인젝션)·개인정보 패턴을 모델에 닿기 전에 막고, 시스템 프롬프트 유출·근거 없는 출처 위조 등을 걸러냅니다.
- **결정론적 안전**: 권한·예산·중단 조건은 "모델 판단"이 아니라 **고정된 규칙**으로 동작합니다.
- **신뢰 보정**: 사용자별로 특정 도구를 허용/차단합니다.
- **관측·운영**: 모델별·세션별 비용 추적과 예산 경고, 응답 지연·품질 감시, 실행 단계별 기록(트레이스), 도구 성공률, 실패 재현, 구성 건강검진, 백업/복원, 상태 대시보드·아침 브리핑.
- **사용 표면**: 터미널(CLI, 100개 이상 명령), 웹앱(채팅·음성·할일·기억·캘린더 등 패널), API 서버(HTTP, 다른 에이전트와 연동되는 표준 카드 제공), 위험한 로컬 명령은 격리 샌드박스에서만 실행.

...
- **근거 있는 교차세션 통찰(회고)** — 여러 세션을 가로질러 "요즘 이런 흐름이네" 같은 통찰을 합성하되, **근거가 된 과거 세션을 함께 인용**합니다. 근거가 약하면 단정 대신 "잘 모르겠다"로 내려가, 없는 통찰을 지어내지 않습니다(위 "작업을 보여준다" 게이트가 회고에도 적용). 합성된 통찰은 **저장돼 나중에 다시 띄우거나 능동 알림이 출처와 함께 인용**할 수 있습니다.
...
- **Muse끼리 노하우 나눔(기반 단계)** — 여러 Muse 인스턴스가 또래로 연결돼 배운 노하우(예: 스킬)를 주고받는 **기반**이 들어왔습니다 — 전송은 **허용된 또래에게만·서명되고·시크릿이 가려진** 채로 오갑니다. 데이터가 아니라 노하우를 나누는 방향이며, 아직 일반 사용자 명령으로는 노출되지 않았습니다.

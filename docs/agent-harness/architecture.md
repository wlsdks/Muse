...
**한 줄 결론:** 협업 구조(역할·핸드오프·게이트·검증·관측)는 **탄탄**하지만, **도구 설계 규약·스킬/MCP
통합·디버깅(DX)** 세 칸이 비어 있고, 루프·컨텍스트·권한·메모리는 **부분**입니다. 즉 "팀이 어떻게
일하나"는 잘 잡혔고, "에이전트가 도구·메모리를 어떻게 다루나"의 운영 규약이 다음 채울 곳입니다.

## 4. 다음에 채울 것 (우선순위)

1. **도구 설계 규약(⬜)** — 도구 이름·스키마·에러 신호·반환 형식·위험 등급. 한 shot 선택을 돕는 규칙.

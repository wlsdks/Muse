...
### 열여덟 번째 실측 — 평가자 부분충족 함정(G11) pass^3 (같은 날)

W1 표본 확장: 명백한 오답(G8 null)이 아니라 **그럴듯하지만 틀린** 빌드를 평가자가 잡는지.

- **함정 빌드:** 단어수에 `len(s.split(" "))` — `split`을 쓰니 맞아 보이지만, 인자 있는 split은 연속/앞뒤
  공백을 빈 토큰으로 세고(`"a  b"→3`) 빈 문자열에 1을 반환(기준 "연속 공백 무시·빈→0" 위반).
- **결과(3회 동일):** **3회 모두 FAIL** + 매번 어느 기준을 어떻게 어겼는지 구체 근거(`"a  b"→3`·`""→1`).
  "split 썼으니 통과" 식 후한 판정 0. pass^3.
- **확인된 것:** 평가자가 명백한 오답뿐 아니라 **미묘한 부분충족**도 잡아냄 — G8(명백)보다 강한 함정에서도
  만든 자≠판정하는 자 원칙이 후한 통과로 새지 않음.
- **한계:** 한 가지 함정 유형(공백 처리). 다른 미묘 버그류는 다음 표본.

...
- [ ] **D4-S1c2** `muse mcp serve` 태스크 read 툴 (기존 task provider 배선, 상태 필터)
- [ ] **D4-S1c3** MCP 실 stdio subprocess 왕복 계약 테스트(spawn `muse mcp serve`→stdio JSON-RPC initialize/tools/call, InMemory 아닌 실 프로세스)

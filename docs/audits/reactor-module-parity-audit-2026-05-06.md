...
| `approval` | `packages/policy`, `packages/runtime-state`, `packages/tools` | Complete | Approval policy, pending approval stores, approve/reject routes, execution gate before risky tools, redacted request rendering. | None; Atlassian-specific resolvers are intentionally not migrated. |
| `auth` | `packages/auth`, `apps/api`, `packages/db` | Complete | Password auth, JWT, revocation, admin bootstrap, admin roles, rate limiting, DB/in-memory user stores, Reactor auth aliases. | None; IAM exchange is intentionally disabled until provider contracts exist. |
...
3. Teams/Atlassian/IAM-specific enterprise integrations that need real customer/provider contracts.

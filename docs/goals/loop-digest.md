...
> ✅ **자율 리뷰관문 (fires 37–39, 진안 묻지 않음):** 사이클9 = 2 ship + 1 defer(전부 공개 arXiv): 37 ISR-LLM 실행-前 플랜 인자검증+1회 수리(2308.13724)·38 self-consistency reverify k-합의(2510.27106+2203.11171, fabrication floor 직접 강화)·39 MAST 질문-관련성 게이트(2503.13657, **DEFERRED** — lexical 신호가 same-script 패러프레이즈 정직 peer false-drop, judge 5/5). **★ESCALATE: 2연속 council-screening defer(36 injection + 39 relevance) 동일 root = lexical/pattern 신호가 다국어+패러프레이즈 over-screen** → 언블로킹 전제는 council 경로 embedding 의미유사도 primitive 먼저. 모델 티어: Fable5가 세션 내내 미가용 → scout/judge 모두 Opus 4.8 fallback(maker=Sonnet 유지, maker≠judge 무결). **사이클10 방향(스스로): lexical council-screening OFF; (a) embedding 의미유사도 primitive를 council 경로에 까는 슬라이스(2개 defer 언블록) 또는 (b) council 아닌 신규 영역(비-lexical recall/orchestration/verification).** fires-37-38 배치 로컬 main 머지 완료(b68ce774, -r build green, push 안 함); 39 defer(코드 무변동).

## [cognition loop] fire 40 — 2026-06-13 · 사이클10 · 테마: 의미유사도 council 인프라 (PAPER-GROUNDED) — 2 defer 언블록

- **무엇:** **Cleanse (arXiv:2507.14649, Joo & Cho 2025 — lexical 대신 임베딩 의미일관성)** 적용 — council outlier 스크린의 SUPPORT 신호를 Jaccard 토큰겹침→임베딩 cosine으로. `councilMemberSupportsSemantic(utterances, embed)`(멤버별 평균 pairwise cosineSimilarity) + `CouncilModelOptions.embed?` 주입(reverify seam 패턴) + `screenCouncilOutliers`가 cosine 시 cosine-전용 `COSINE_ABS_FLOOR=0.4`(Jaccard 0.08은 cosine서 inert). CLI 라이브(commands-swarm :473) 실제 임베더 주입. fire-31 consensus는 Jaccard 유지(backlog).
- **왜:** 2연속 council-screening defer(36 injection·39 relevance)의 ROOT = lexical 신호가 다국어/패러프레이즈 over-screen. cosine은 표면형 무관 의미일치 점수 → cross-lingual 합의 peer false-drop 제거(fire-28 한계 해소). 임베더가 이미 council 경로 도달(council-corpus.ts)·synthesizeCouncilAnswer가 이미 async+주입클로저라 plumbing 아닌 loop-sized. KIND=의미유사도 인프라(저-floor-risk, lexical 아님 — tar-pit 회피). 항상-on은 아니나 muse swarm council on-invocation(28/31/RGV와 동급 liveness).
- **리뷰지점:** council.ts(councilMemberSupportsSemantic+COSINE_ABS_FLOOR+precomputed supports 경로)+index.ts·commands-swarm.ts(라이브 embed 주입)+program.ts(councilSynthesisOverride seam). **maker=Sonnet / judge=Opus 4.8(Fable5 세션 미가용) PASS**: 독립 2-revert로 non-inert(Jaccard-on-embed→deceptive-KO-caught flip; absFloor=0.08→floor 테스트 flip, 둘 다 단일 COSINE_ABS_FLOOR); seam이 REAL screen 구동(short-circuit 아님); 0.4 floor가 valid divergent peer(cosine 0.45) KEEP(relFloor+majority-cap 경계, fire-28 대비 retention 개선); fail-open(embed throw→Jaccard)·back-compat byte-identical·fire-31 무변. agent-core 1924·cli 2554·check 0·lint 0.
- **리스크:** 낮음 — subtractive selection, fail-open, 임베딩 LOCAL(a2a wire 무변), downstream RGV+citation 게이트 後 불변, fabrication=0 untouched. **정직한 caveat(judge):** KO+EN 합의 fix-테스트 일부 VACUOUS(Jaccard all-zero→relFloor×0 가드가 양쪽서 keep) — fix는 합성으로 증명(agreeing→high cosine unit + deceptive-catch revert)되나 majority-KO+minority-EN 차별 fixture는 backlog. COSINE_ABS_FLOOR=0.4는 live KO/EN battery 튜닝 backlog(smoke:live stall로 미검증). RATCHET: testFiles +0(다수 케이스), fabrication 0, 신규: council 의미유사도 primitive(fire-39 relevance + fire-36 injection 재시도 언블록). grounding floor 무관.

## [cognition loop] fire 41 — 2026-06-13 · 사이클10 · 테마: council 의미 질문-관련성 게이트 (PAPER-GROUNDED) — fire-39 언블록 완료

- **무엇:** fire-39 질문-관련성 게이트를 **의미 cosine으로 재구현** — `screenOffTopicUtterancesSemantic(question, utterances, embed)`: cosine(질문, reasoning) < QUESTION_RELEVANCE_FLOOR(0.3)면 off-topic 드롭. fire-40의 주입 embed 재사용, synthesizeCouncilAnswer에 dedupe 後·outlier 스크린 前 배선. NO dominantScriptFamily 가드(의미 cosine이 cross-lingual 네이티브 처리), NO lexical fallback(lexical이 fire-39 실패 원인). MAST FM-2.3(2503.13657) + Cleanse(2507.14649).

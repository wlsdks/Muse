...
- ✓ Playbook eviction PEVI-parity (edge c, Wilson LCB, PEVI arXiv:2012.15085) — paper-grounded fire 4 (`65a12bc9`); corrected fire-3 rollback (wrong function replicated), discriminating test added
- ◦ Cross-package Playbook parity test: eviction's inline Wilson-LCB (packages/mcp) hand-mirrors agent-core's `rankingUtility`/`wilsonInterval`; a same-(r,d)→same-utility test spanning both packages would fail-close on silent drift if agent-core's formula changes (judge-flagged residual risk from fire 4).

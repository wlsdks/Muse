...
- ✓ desktop 음성모델 다운로드 % 버블 truncation("99%" 고착)·sub-1% "0%" 노이즈·no-clamp("101%") + NaN trap — downloadProgressBubble(round+clamp+≥1%, NaN-safe) — surfaces fire 54 (`(pending)`)
- ◦ Playbook eviction PEVI-parity (edge c, PEVI arXiv:2012.15085): `retainPlaybookEntries` (packages/mcp/src/personal-playbook-store.ts) sorts bank-overflow survival on RAW point-estimate reward while the injection path ranks on `rankingUtility` (Wilson LCB, playbook.ts:383) — a thin-but-lucky strategy destructively evicts a battle-tested one. FIX: inline-replicate **`rankingUtility` (Wilson LCB)**, NOT `effectiveStrategyReward` (shrinkage — paper-grounded fire 3 rolled back for replicating the wrong function). No-tally entry falls back byte-identically to clampReward(reward). Add a DISCRIMINATING test (thin 1/0 vs proven 11/9: survives only under Wilson, not shrinkage). Gate: `pnpm --filter @muse/mcp test` + `pnpm self-eval`.
...
### Refill batch 3 (gap-scout fires 19-26)

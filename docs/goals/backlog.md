...
- ⏳ **★CRITICAL — Korean (hangul) faithfulness 0/4 = fabrication-floor breach, BLOCKS ALL PUSHES (grounding/recall domain, NOT tool-hardening — needs grounding loop or 진안)** — `pnpm precheck:grounding` (live pre-push tripwire) FAILs consistently (re-ran, not flaky): `hangul faithfulness 0.00 (0/4)` vs `latin faithfulness 0.94 (16/17)`. Korean answerable cases (월세/엔진오일/헬스장) answer citing a source that was NOT retrieved (Korean retrieval fails to surface it); Korean refuse cases (혈액형/식비/비밀번호/미용실) retrieve `confident` when they should abstain — mis-calibrated both ways; English fine. **fire-55 suspected ca7b1863 (recall refactor) — DISPROVEN: it's a clean byte-identical relocate of `buildAskConnections` (the `--connect` footer, NOT the faithfulness path).** True cause unknown; the hangul arm is generated in a shared grounding-eval module (not precheck-grounding.mjs) — likely a Korean retrieval/embedder/grounding-gate calibration issue. 2 fires running; `MUSE_SKIP_PREPUSH` MUST NOT be used (IMMUTABLE-CORE). tool-hardening commits pile in local main behind this until it clears.

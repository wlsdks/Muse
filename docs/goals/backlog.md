...
### Refill batch 3 (gap-scout fires 19-31)
...
- ★ **wrong-tool selection on the file-fix task — ROOT-CAUSED (computer-control fires 1-3, 2026-06-20), fix is fuzzy >1-fire.** `eval:computer-task` (buggy-`add` fix) FAILS 2/2 STABLE with `tools=[muse.skills.run/list, muse.context.fetch/list]` and file LEFT UNEDITED — the 12B never calls file_read/file_edit. PRECISE root cause: `muse.context.*` + `muse.skills.*` (6 tools) all carry **domain="core" → isMandatoryTool=true** (`tool-filter.ts:221`), so they are ALWAYS-ON (cap-protected) on EVERY turn — including a file-fix task where they're pure distractors. The runtime exposes ~20 tools (3× tool-calling.md's ≤5-7), and the 12B picks the prominent always-on skills/context over the file tools. NOT eval-unrealism (production's `planForContext`→`capToolsByRelevance` is exercised) and NOT a mis-classification (skills/context being core is a deliberate "always reachable" design). FIX OPTIONS, both fuzzy + stochastic-to-verify (each eval run ~7min × pass^k): (1) intent-classification to hide skills/context for a detected file/actuator task; (2) a path-mention relevance boost so file tools out-rank (but mandatory distractors remain). NOT the clean deterministic repair class the theme premised (literal-\n / scope-default — already shipped + exhausted). NEEDS DELIBERATE human-paced effort (dedicated eval time + adaptive-judge safety net), not a 15-min auto-loop fire. Detail in `docs/goals/loops/computer-control.md`.

...
- ◦ memory key-normalization asymmetry InMemory/Kysely vs File (fire-143 scout): `FileUserMemoryStore.upsertFact` normalizes the key (normalizeMemoryKey) but InMemory/Kysely `upsertFact` store the RAW key, so the same fact fragments into two entries depending on the store backend, and auto-extract's `current` lookup (normalized) can miss a raw-stored value and mis-classify UPDATE as ADD on non-file stores. The File store is the production CLI path (so 진안 unaffected), but normalize uniformly across all three impls. (judge/scout-noted fire 143)

...
- ◦ performConsentedAction action-log omission (gap-scout follow-up, secondary): unlike every send path (rule 4), `performConsentedAction` writes NO action-log entry on any branch (performed/refused). No live effect yet (no production caller), but should append a rationale-bearing entry (sent OR refused) before it's wired into a standing-objective loop.
...
- ★ (G1) **Surface the `source-conflict` (+ other unsurfaced) weakness axis for remediation** — SURFACE self-development · size S-M. `WeaknessAxis` has `source-conflict` (`@muse/mcp/weakness-ledger.ts:33`) and it IS written to the ledger (`recordAskWeakness`, recall/weakness.ts:131), but NEITHER selector reads it: `selectRemediableWeaknesses` filters only `grounding-gap` (weakness-ledger.ts:234); `DEV_FIXABLE_AXES` (weakness-ledger.ts:252) = {misgrounding, unbacked-action, wrong-tool, time-parse} — `source-conflict` in neither → detected-but-never-remediated dead-end fuel (the fire-5 residual, now scoped). VALUE: closes the whetstone monitor→remediate loop for the conflict axis (the one tied to the grounding edge). SEAM: weakness-ledger.ts selectors + recap/doctor consumers; decide user-remediable ("reconcile your conflicting notes about X") vs dev-fixable; test the axis flows end-to-end through the chosen selector.
- ★ (G2) **Unify the injection neutralizers — give the live-surface neutralizer policy's evasion-decoding** — SURFACE grounding-floor · size M. agent-core's `neutralizeInjectionSpans` (applied to ALL tool/MCP/sub-agent output via `capToolOutput`) only zero-width-strips + regex (`agent-core/src/injection.ts:64`); it does NOT homoglyph-fold / HTML-entity-decode / NFD-normalize. `@muse/policy`'s `normalizeForInjectionDetection` does ALL THREE but only for USER input (`policy/src/injection-patterns.ts:154-283`). So a homoglyph-`іgnore` / `&#105;gnore` injection in tool output EVADES the live defense. FEASIBLE: agent-core already depends on @muse/policy (no cycle), which re-exports the normalizer. VALUE: closes a real bypass on the highest-risk surface. SEAM: run `normalizeForInjectionDetection` (or a shared subset) inside `neutralizeInjectionSpans`/`isMemoryInjection`; test homoglyph + entity + NFD injection in tool output neutralized, clean byte-identical. (This is T1-c-resid ii, now first-class.)
- ★ (G3) **Per-fact provenance on `UserMemory.facts` (firstSeen / lastConfirmed / recallCount)** — SURFACE memory-integrity · size L. `UserMemory.facts` is flat `Record<string,string>` (`@muse/memory/src/index.ts:298`), only store-level `updatedAt` + `factHistory` for replaced facts; grep `recallCount|lastConfirmed|firstSeen` in memory/src = ZERO. `recall-promotion.ts` operates on EPISODIC records, not the user-fact store. VALUE: the named prerequisite (was T1-b-i) unblocking G4 + a fact freshness/staleness signal ("this fact is 8 months stale — re-confirm?"). SEAM: `@muse/memory` UserMemory/UserMemoryStore types + both store impls (file + Kysely), parallel `factMeta` map keeps `facts` back-compat, one-time forward migration (legacy facts default provenance), wire ONE consumer end-to-end (recall increments recallCount) so the OUTCOME is testable. Risky migration — never lose a fact.
- ◦ (G4) **Memory-promotion gate — durable user-fact promotion from gated recurrent recall** — SURFACE memory-integrity · size M · DEPENDS ON G3. grep `promoteMemory|shouldPromote` = ZERO; `selectPromotableMemories` (recall-promotion.ts:84) promotes EPISODES by hit-count, NOT facts into durable `UserMemory.facts`, and no gate decides when a reconfirmed fact earns durable status. VALUE: turns the fact store self-curating (confirmed facts get durable) — memory-integrity + higher-confidence grounding. SEAM: a deterministic fail-close gate in @muse/memory consuming G3's recallCount/lastConfirmed (never auto-promote an un-reconfirmed or injection-flagged value — reuse `defangMemoryInjection`); test promotes only past threshold, no partial side-effects on deny.
- ◦ (G5) **Chat-path auto-orchestration parity with `ask`** — SURFACE orchestration · size M. `runDecomposedAgentAsk`/`shouldDecompose`/`runLeadWorkerTask` callers are ONLY ask-decompose.ts + commands-ask.ts (codegraph_callers); `chat-repl.ts` has ZERO decompose/lead-worker references → a multi-step `muse chat` request is NOT decomposed into context-isolated sub-agents the way `ask` is (CLAUDE.md non-negotiable: don't fork agent behavior across surfaces). VALUE: closes the ask/chat orchestration fork. SEAM: chat-repl.ts model-invocation path reusing `runDecomposedAgentAsk`. CAVEAT (planner pass FIRST): chat is a streaming continuous-companion REPL, not single-shot — decomposition must respect streaming + history. Lower-ranked for this design question.

### ✓ verified ALREADY-BUILT (do NOT re-scout — gap-scout fire 8): skill curator (curate/consolidate/held-out gate, @muse/skills) · playbook reward/decay RL (PEVI LCB ranking, agent-core/playbook.ts) · ask→sub-agent orchestration (runDecomposedAgentAsk fully wired, commands-ask.ts:551).
...
- ◦ (T1-a-resid) the `^system:` pattern is start-of-string anchored (no `m` flag), so an injected `\nsystem:` line MID-prose isn't caught (Opus judge fire-3 note) — small line-anchored-pattern follow-up; low priority (the other 3 patterns catch the common shapes).
...
- ◦ (T1-c-resid ii) two adjacent evasions stay OPEN on the 7 grounding/tool surfaces (Opus judge fire-7 note): HTML-entity-encoded (`igno&#x200b;re`) + NFKC-homoglyph (fullwidth `ｉ`) still bypass `MEMORY_INJECTION_PATTERNS` because this path does NOT entity-decode or NFKC-normalize. `@muse/policy`'s `normalizeForInjectionDetection` (user-input surface) ALREADY does all three — so the highest-value fix is to UNIFY: have the agent-core grounding/tool path reuse (or mirror) the policy normalizer, closing entity/homoglyph evasion everywhere with one source. (Defer if fire 8 pivots off the injection theme for lever diversity.)
...
- ◦ (T3-b) Ollama/llama.cpp GBNF schema sanitizer — normalize tool JSON-schemas the local backend's strict grammar rejects (bare {type:object}, array types, anyOf-null, $ref-with-siblings). Pattern: hermes `schema_sanitizer.py`. Muse seam: Ollama adapter tool-schema projection (hardens the local path directly).

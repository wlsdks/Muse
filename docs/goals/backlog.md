...
- ◦ web-url-guard NAT64: the `64:ff9b::/96` prefix (upper bits 0064:ff9b, neither 0 nor ffff) embeds an IPv4 in its low 32 bits the fire-108 parser does NOT decode — `[64:ff9b::7f00:1]` (NAT64 of 127.0.0.1) is still public. Narrower vector (needs a NAT64 gateway reachable) but a complete SSRF guard would catch it. Add the NAT64 prefix to the embedded-IPv4 check. — tool-hardening (candidate, fire 108 follow-up)

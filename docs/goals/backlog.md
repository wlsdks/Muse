...
- ★ multi-hop recall wiring — DECOMPOSED (capability-boost fire 1): ask의 notes recall이 `rankKnowledgeChunks(WithHop)`을 안 쓰고 자체 인라인 cosine+hybrid(`apps/cli/src/commands-ask.ts:1001-1078`, seedMatches@1078)임을 확인. `rankKnowledgeChunksWithHop`(`packages/agent-core/src/knowledge-recall.ts:1144`, secondHop+associative, AUGMENT-never-displace)은 완전 빌드+테스트됐고 `knowledge-corpus.ts:485/582`가 `MUSE_RECALL_SECOND_HOP` env-gated로 호출하나 ask 경로 미적용. 측정 ROI 양성(`eval:multihop` single-hop two-hop hit@4 2/5=40%). >1 fire wedge-critical이라 loop-sized 슬라이스로 분해:
  ...
  - (1c NEXT) `eval:multihop`을 same-base A/B control(inline path WITHOUT second-hop)까지 보고 + default-on promotion(또는 `muse ask` config flag로 노출 — 현재 default-off staging이라 프로덕션 미활성) + CI 가드 등록. judge가 "permanently-dark feature 방지" flag한 후속.
  순서 1c(promotion+가드). org.md(5케이스 중 1)는 여전히 miss — 더 깊은 hop 또는 corpus별 튜닝 후속.
...
- ◦ csv_parse duplicate-header collapse (fire-148 judge-noted, pre-existing): two columns sharing a header name (or two empty "" headers) collapse — the later `row[name]` overwrites the earlier, silently losing a cell. Same data-loss class as the ragged-long fix but a different facet (key collision, not count mismatch); needs a de-dup/suffix on duplicate header names. Not introduced fire 148.
...
- ◦ non-digit named-entity negated fabrication (fire-146 residual = existing named-entity drift): a hedge whose negated claim has NO digit ("…your manager isn't Alice, it's Bob") still slips as pure — the digit heuristic only catches time/number fabrications. Pre-existing (not introduced fire 146), and the same class as the long-open "named-entity drift on chat" (see MEMORY chat-gate). Needs a positive-named-entity / NLI signal, not a digit. Also `안\s`/`못\s` miss no-space `안돼`/`못해`. (judge-noted fires 140/146)

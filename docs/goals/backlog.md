...
- ◦ widen answerIsPureRefusal clause split (residual hole, fire-137 judge note): CLAUSE_SPLIT_RE splits on sentence-terminators + adversatives (but/however/…), so a hedge joined by em-dash (—), colon (:), parenthesis, or a BARE comma without an adversative ("I don't have it, your flight is 9am") still rides through as grounded. NOT a regression (old substring code did too) — a widening. Extend the separator set (incl. comma/—/:) weighed against the false-warning cost on benign comma-clauses, OR invert to a refusal-clause-removal token-density test. (scouted fire 137)

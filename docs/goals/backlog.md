...
- ◦ korean_number reverse (Korean WORDS → digits, '1억 2천만' → 120000000): korean_number (fire 131) does digits→Korean; the inverse parses a Korean myriad expression (만/억/조 + 천/백/십 sub-units) back to an integer — the 12B is also unreliable parsing compound forms. More parse edges (천/백/십 within a chunk); deterministic. — tool-hardening (candidate, fire 131 follow-up)

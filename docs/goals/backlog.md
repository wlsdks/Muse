...
- ◦ date-diff cross-year-roll bypasses realDate (fire-141 judge residual, same bug class): `detectDateDiffQuery` line ~86 rolls a year-less end date that sorts before the start via `new Date(b.year+1, b.month, b.day)` WITHOUT validating — so "from March 1 to February 29" with now in a leap year parses Feb 29 (valid), rolls to Feb 29 next (non-leap) year → silently Mar 1 → wrong count. Pre-existing, NOT a regression, extremely narrow (year-less Feb 29 sorting before start, leap now-year). Apply realDate to the roll → null, or roll to the next actual leap year. (judge-noted fire 141)
...
- ◦ negation-DOMINATED refinement (fire-140 judge residual): the negation skip is per-clause-contains-a-negation, so a hedge whose tacked-on claim CONTAINS a negation ("I don't have it, but your meeting is NOT at 3pm, it's at 4pm in room 5") now slips as pure (fire-137 caught it) — a narrow unsafe-direction residual (rarer than the positive-hedge form, which is still caught). Tighten: skip a clause only if it's negation-DOMINATED / lacks a residual positive assertion (e.g. a length/positive-data cap), not if it merely contains one negation token. Also `안\s`/`못\s` miss no-space `안돼`/`못해`. (judge-noted fire 140)
...
- ◦ weather geocode ambiguity (scouted fire 135): OpenMeteoWeatherProvider.geocode silently uses results[0] (count=1), so 'Springfield'/'Paris' resolves to open-meteo's top hit with no signal which city was chosen — possible wrong-place weather. Lower value (no fail-close violation), but surfacing the resolved name+country for confirmation would harden it.
...
- ◦ BLOCKER (embedder cohere, codebase-quality fire 98 — reverted): consolidating createOllamaEmbedder + createGateEmbedder from context-engineering-builders.ts INTO embedder-base.ts (their natural home next to resolveEmbedderBase; reunites buildEpisodicRecallProvider's separated doc; removes 4 now-stale imports; no cycle — embedder-base is dependency-free) is a clean refactor BUT trips the egressGuards ratchet: scripts/self-eval.mjs `egressSources` HARDCODES context-engineering-builders.ts and counts `throw new LocalOnlyViolationError(` sites there; moving createOllamaEmbedder's guard to embedder-base.ts drops the count 7→6 (the guard still fires — behavior preserved — but the counter doesn't follow it). Doing this needs a human to add `packages/autoconfigure/src/embedder-base.ts` to egressSources (self-eval.mjs is human-directed infra, off-limits to the loop). Reverted this fire; pick up once egressSources is updated.

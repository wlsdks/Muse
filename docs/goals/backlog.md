...
- ◦ **macos-tools capture cluster decompose — Step 2 (fire 45 did Step 1)** — the path-validator sandbox now lives in macos-screen-path.ts. Step 2: move createMacScreenshotTool + createMacScreenReadTool (+ MacScreenshotToolDeps/MacScreenReadToolDeps + SCREENSHOT_TIMEOUT_MS + SCREENCAPTURE_PATH) into a new macos-screen-tools.ts sibling (import resolveScreenshotPath/tryRealpath from macos-screen-path.js + runChild from macos-exec.js), re-export from macos-tools (like fire 43's utility-tools split). screenread uses deps.describeImage (vision callback, no import).

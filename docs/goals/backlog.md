...
- ✓ Salience-weighted eviction for the REFLECTION store (Generative Agents arXiv:2304.03442): `scoreReflectionRetention`/`selectRetainedReflections` (@muse/mcp reflections-store) trim cap-overflow by recency+salience(min(1,support/5)) not pure recency, so a high-support recurring insight isn't evicted for a thinner newer one; equal-support → legacy recency — self-improvement fire 11 `c9e7fe4b`
  - ◦ FOLLOW-UP: `listReflections` still surfaces newest-first, so a RETAINED high-support old insight may sit below newer ones at the ask-grounding surface (retention≠display; strictly better than before since it now survives at all).
...
- ◦ **Memory factHistory refine-vs-contradict LABELING** (Mem0 arXiv:2504.19413; fire-16 scout). NOTE: the contested/volatility HALF is ALREADY shipped — `refinementAwareDistinctValueCount` (belief-provenance-store) token-subset-excludes refinements so a refinement never marks a fact contested. Remaining = the `muse memory` TIMELINE conflates refine ("Seoul"→"Seoul, Gangnam-gu") with contradict ("Seoul"→"Busan") because `collectFactSupersessions` (memory-user-store:90) uses `prior !== next` string-ineq. Decomposed (DON'T just drop refinements from factHistory — that loses elaboration history; LABEL them):
  - ◦ (a) @muse/memory: `FactSupersession` gains optional `kind: "refine"|"contradict"`, set via `classifyValueChange` in `collectFactSupersessions`; back-compat (absent on legacy). Persist round-trip in BOTH `memory-user-store.ts` (InMemory) + `memory-user-store-file.ts` (File serialize/deserialize). OUTCOME test: a refine logs kind="refine", a contradict kind="contradict".
  - ◦ (b) @muse/cli: `buildFactTimeline`/renderer (commands-memory.ts:325) shows the kind — "refined to" vs "changed to". OUTCOME test: timeline output distinguishes the two.
...
- ★ **EFFICACY AUDIT findings (fire 17, 3 Opus audits, codegraph-verified)** — answering "is self-improvement REAL / are the mechanisms correct?". HEALTHY+live-default: whetstone ledger, Mem0 auto-extract + belief-provenance (File store), Playbook ranking (rankingUtility Wilson-LCB, no regression), skill select/usage/eviction. Open findings ranked:
  - ◦ **(HIGHEST) Cross-turn "experience-helps" DELTA eval** — NO eval anywhere proves turn-N+1 is better BECAUSE of turn-N's stored experience; every self-improving battery is single-turn mechanism-firing, self-eval is a count ratchet (bigger≠better). Build the self-improvement analog of `eval:grounding-delta`: same fixed tasks run with EMPTY store vs PRIMED store, assert primed arm grounds+cites where empty arm abstains. Needs a `prime(store)` scenario seam in `runEvalSuite` (scripts/eval-harness.mjs) + `verify-experience-delta.mjs` in BATTERIES. Deterministic, mockable, pass^k, citation-required (guards GROUNDED≠TRUE). LIVE (Ollama) — pre-verify STABLE 3/3.
  ...
  - ◦ **Summary-store recall INERT in CLI** — `buildEpisodicRecallProvider` is "default-on" but reads `InMemoryConversationSummaryStore` (no DB in CLI) → empty every process → no cross-session recall AND starves the recall-hits ledger → fade/promotion consolidation get zero fuel. Add a `FileConversationSummaryStore` (mirror `FileBeliefProvenanceStore`) used when `db` is undefined. Verify 2-process: session-1 writes a summary, session-2 recalls it.
  - NOTE: fire-17 audit Finding "buildPlaybookProvider drops origin" was REJECTED (hallucinated cite — function doesn't exist; CLI carries origin via `toPlaybookStrategy`). Don't action it.
- ◦ **Playbook reinforcement-credit by INJECTED-ID set** (precision gap, fire-16 scout). `applyPlaybook` (playbook.ts:923) records only `metadata.playbookApplied:true`, NOT which strategy ids were injected, so the decay/credit path `moveReward` (chat-distill-corrections:141) RE-DERIVES the target by cosine (`selectCreditTargetSemantic`) — a correction can decay the WRONG strategy. Decomposed (3 seams):
  - ◦ (a) @muse/agent-core: `applyPlaybook` records the injected strategy id list in run metadata (additive).
  - ◦ (b) @muse/cli: session-history persists that id list across the turn.
...
### Refill batch 3 (gap-scout fires 19-35)
...
- ◦ broaden source-conflict value extraction — DECOMPOSE-ON-DEFER (core-hardening fire 9 analysis): the `label: value` regex truncates at the first comma (`Address: 12 Baker St, London` → "12 Baker St"), so "…, London" vs "…, Paris" both truncate to "12 Baker St" = a real conflict MISSED (false-negative). BUT naively allowing commas ADDS a NEW false-positive class: a comma-LIST sharing a first element (`items: a, b` vs `items: a, c` → currently both "a" = no conflict; broadened → "a, b" vs "a, c" = spurious conflict). So broadening trades a false-negative for a false-positive — NOT a clean 1-fire regex tweak. Needs a list-vs-single-value heuristic (an address is one coherent comma-value; a list is N short comma-tokens) + the fire-6 rigor (LARGE benign Korean+English corpus, STABLE 0 false-positives) before landing. >1 fire; decompose: (a) design the list-vs-value discriminator + benign corpus, (b) apply + verify. (analysis fires 7-9)

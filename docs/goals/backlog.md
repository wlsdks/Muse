...
- ◦ widen answerIsPureRefusal — needs the INVERT approach, NOT a separator regex tweak (fire-139 BLOCKED finding): tried extending CLAUSE_SPLIT_RE with em-dash/colon → it REGRESSES pure refusals. A refusal's own continuation that crosses the new separator ("I'm not sure — that isn't in your notes") is misread as a hedge because the clause-level `answerIsRefusal` markers don't cover the contraction continuation ("isn't in your notes" ≠ the marker "not in your notes") → false "treat as unverified" warning on an honest refusal. Widening separators just exposes the marker detector's gaps (whack-a-mole). The robust fix is the judge's INVERT idea: detect the post-refusal content is a NEW factual assertion (not a refusal restatement) — a semantic/structural check, not more separators. The residual (em-dash/colon/bare-comma hedges) stays open but is NOT a regression (old substring code passed them too). Rolled back fire 139; needs careful design (>1 fire or dedicated). (scouted fire 137, BLOCKED fire 139)

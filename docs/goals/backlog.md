...
- ◦ lunar→solar (음력→양력, for lunar birthdays): lunar_date (fire 126) does solar→lunar; the inverse ("내 음력 생일 5월 5일이 올해 양력으로 며칠?") needs a bounded search over the solar year formatting each day in dangi to match the target lunar M/D, with leap-month disambiguation. Deterministic (same ICU authority) but more code + edge cases. — tool-hardening (candidate, fire 126 follow-up)
...
- ◦ **preference supersession: wire the CLI twin inferSessionPreferences** (apps/cli/commands-user.ts) — fire 47 wired belief-revision supersession into the autoconfigure/daemon arm only; the interactive CLI session-end path (chat-ink → inferSessionPreferences) duplicates the upsert loop and still accumulates cross-category contradictions. Wire findSupersededPreferenceId there too (or refactor it to delegate to inferPreferencesFromTurns). DEFAULT_PREFERENCE_SUPERSEDE_MAX=6 untuned. (agent-core-cognition fire 47 caveat)

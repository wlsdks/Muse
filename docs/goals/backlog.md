...
- ◦ (H3) **Semantic / cross-lingual source-conflict detection** — SURFACE grounding-floor · M. `detectSourceConflict` (recall/conflict.ts:123) is PURELY lexical (`LABELLED_VALUE` `[A-Za-z]:value` only) — misses prose contradictions, cross-lingual field disagreement, and Hangul-labelled fields. SEAM: embedding-backed second pass (reuse `detectEvidenceContradictions`' topic-sim + neither-subset) → feed the existing `contestedOutcome`. OUTCOME: opposite-value prose notes → contested; KO vs EN field disagreement → detected; elaboration/superset does NOT fire. fail-open. pass^k=3 on the live cross-lingual case.

...
- ◦ vision cross-value field-role grounding (grounded-vision fire-4 residual): fieldIsGrounded still false-passes a hallucinated value that coincidentally matches a DIFFERENT field's evidence run — e.g. `fieldIsGrounded("$2026", "Concert 2026 … $40")` → true (a hallucinated TOTAL grounding on a coincidental YEAR). Needs field-role semantics (a `total` must ground on an amount-shaped run, not any digit run), which the per-value gate lacks. Deterministic-testable in vision-actions.test.ts. Gate: `pnpm --filter @muse/cli test`.
...
### Refill batch 3 (gap-scout fires 19-32)

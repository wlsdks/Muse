...
- ◦ web-url-guard: the DEPRECATED IPv4-compatible IPv6 form (`::127.0.0.1` → `::7f00:1`, no `ffff:` prefix) is not decoded by isPrivateIPv6 — surfaced as a non-blocking note in the fire-105 ④b judge. Low risk (RFC 4291-deprecated, generally unroutable) but a thorough SSRF guard would classify it too. Confirm whether `new URL()` ever emits it and whether any host reaches it before building. — tool-hardening (candidate, fire 105 follow-up)

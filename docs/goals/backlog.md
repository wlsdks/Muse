...
- ◦ (T1-a-ko-resid) Korean injection coverage is the ignore/forget shape ONLY — the act-as/role-hijack (`너는 이제 …처럼 행동해`) and output-clamp (`…만 답해`) Korean analogs of English patterns 2-3 are NOT yet covered (future-additive; within the accepted-collateral envelope). Low priority.

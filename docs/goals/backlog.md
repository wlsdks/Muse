...
- ◦ remaining clean ◦ for next fires: macos system_set extraction (last non-outbound family, ~95 LOC, cycle-free — decompose); cli dead-code batch in stable files (program-helpers.ts friendlyFetchError/isNodeError, commands-today DEFAULT_TODAY_HEADLINES_CAP re-export — triage each)

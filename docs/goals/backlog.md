...
- ◦ `/remember key=value` slash command has the SAME ASCII-slug bug (apps/cli/src/chat-ink-core.ts `parseRememberArg`) — `/remember 취미=등산` drops the Korean key; same root cause, fix the same way (delegate to normalizeMemoryKey). Non-model-facing CLI path. (scouted tool-hardening fire 128)

...
- ◦ **add_contact `phone` arg-grounding (fabrication floor on the highest-harm field)** — `add_contact` (contacts-tool.ts:106) grounds only `relationship`; a model-fabricated `phone` the user never stated gets PERSISTED → future messages misdelivered to a wrong number. `phone`'s digit-tokens are highly specific (a fabricated number won't coincidentally ground), so adding `"phone"` to `groundedArgs` lets `groundToolArguments` reliably drop fabrications while keeping a stated number (even reformatted +country/spaces — digit runs survive). FINDING (fire 64): `email`/`handle` are NOT good grounding targets under the ANY-token mechanism — their local-part (`bob@…`, `@bob`) = the contact NAME which is in the utterance → a fabricated domain false-grounds via the name token. `birthday` is reformatted (MM-DD) → brittle. So ground ONLY `phone`. Test home wrinkle: the grounding mechanism + its tests live in @muse/agent-core (no mcp↔agent-core dep), so the behavioral proof goes in `agent-core/test/tool-argument-grounding.test.ts` (add a digit-token/phone case — none exists) + the metadata `"phone"` in mcp. Deferred from fire 64 (agent-core is the concurrent agent-core-enhance loop's hot package — land when it quiets, or as a paired 2-file slice).
...
## ⚠️ CORE-EDGE REGRESSION (blocks repo-wide `pnpm check`) — owner: cognition loop

- ★⏳ **add_contact arg-grounding DROPS a user-stated phone** — `apps/cli/src/actuator-tools.test.ts > add_contact ... > "KEEPS a phone the user actually stated"` + "DROPS a fabricated phone" both FAIL (phone undefined / contact length 0). Fails in ISOLATION after a clean rebuild → real regression, not stale-dist. Bisected to **`5ec47842 fix(agent-core): groundToolArguments reports only fully-dropped args, not partial-array cleans (cognition loop fire 21)`** — the `groundToolArguments` change over-drops a *grounded* value (anti-fabrication gate too aggressive = Muse's core edge eroding). NOT a test-hygiene fix (it's the cognition loop's product-logic change); recorded here so the owner loop repairs it. (found test-hygiene fire 9)
...
- ◦ **extend nav-status fidelity to post-click/type navigations** — fire 9 shipped httpStatus/statusError for browser_open + browser_back (the real goto/goBack HTTPResponse). The act paths (click/type) still don't capture nav status because the real click doesn't observe the document HTTPResponse; wiring it needs a main-frame page.once("response") race. Lower value than open/back (covered) — only matters when a click itself navigates to an error page. (fire-9 follow-up)

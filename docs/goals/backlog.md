...
- ◦ **Council consensus-advisory remainder** — (a) derive the consensus floor from the support computation that ACTUALLY ran, not options.embed: if councilMemberSupportsSemantic throws and keptSupports falls back to Jaccard, the cosine floor (0.5) is wrongly applied → over-reports "weak" (rare catch-path, advisory-only so over-caution is the safe direction, but fix the source-of-truth); (b) tune the two floors (0.5 cosine / 0.16 Jaccard, reused from ReConcile) on a real council support distribution. (fire 19 remainder, arXiv:2509.14034)

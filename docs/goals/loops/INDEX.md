...
| `a-plus-roadmap` | A+ 로드맵 §10.3 집행 (main worktree) | [a-plus-roadmap.md](a-plus-roadmap.md) | 33 | `c5e6dab25` | v2.x | active (cron `964ac861`, 30m session-only, Tier1 no-push) |
...
| `response-experience` | 어시스턴트 응답 경험 지속 개선 (worktree `/tmp/muse-response-loop`, branch `response-loop`, verified-fire → main FF push opt-in) | [response-experience.md](response-experience.md) | 7 | `ccc729af6` | v2.x | active (cron `513db0d4`, session-only 20m) |

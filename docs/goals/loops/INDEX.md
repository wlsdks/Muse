...
| `local-speed` | 로컬모델 속도/효율 라우팅(KV-quant·cascade·spec-decode) — 정확성 보존 (worktree `/tmp/muse-local-speed`, branch `loop/local-speed`, Tier2 push + 3-fire main merge) | [local-speed.md](local-speed.md) | 5 | `pending` | v2.0.0 | active (cron `481d865a`, session-only) |

...
| `surfaces` | 제품 표면 강화 — CLI · macOS desktop · web (worktree `/tmp/muse-surfaces`, branch `loop/surfaces`) | [surfaces.md](surfaces.md) | 54 | `2a756c0a` | v2.0.0 | active (manual this session; cron `c5b90c94` deleted — raced same worktree) |
...
| `context-strategy` | Context engineering — leanest sufficient context per turn (worktree `/tmp/muse-context-strategy`, branch `loop/context-strategy`) | [context-strategy.md](context-strategy.md) | 8 | `a056e546` | v2.0.0 | active (cron `c66c8b81`, session-only, Tier2 push+PR, merge-to-main every 5 fires) |

...
| `surfaces` | 제품 표면 강화 — CLI · macOS desktop · web · web콘솔 API (worktree `/tmp/muse-surfaces`, branch `loop/surfaces`) | [surfaces.md](surfaces.md) | 63 | `a1f44f18` | v2.0.0 | active (cron `87c1b283`, session-only 20m, /tmp/muse-surfaces; fire 63=Skills 웹뷰, 3배수 main-merge) |
...
| `computer-control` | local-12B multi-step computer-task reliability (eval:computer-task) — retheme from core-hardening (worktree `/tmp/muse-computer-control`, branch `loop/computer-control`, Tier2 push) | [computer-control.md](computer-control.md) | 42 | `8f9066aa` | v2.0 | active (cron `47491301`, session-only) |

...
| `poisoned-source` | GROUNDED≠TRUE 출처-진위 방어 — 오염된 노트/episode/MCP가 grounding 게이트 세탁 못 하게 (worktree `/tmp/muse-poisoned-source`, branch `loop/poisoned-source`, Tier2 push, merge-to-main ea 3 fires) | [poisoned-source.md](poisoned-source.md) | 11 | EP-1b per-turn trust persistence (fail-open closed) | v2.0.0 | active (cron `cb79365d`, session-only) |

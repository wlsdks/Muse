...
## fire 91 · 2026-06-14 · loop-creator v1.14.0 · a1b32a02
meta: value-class=refactor · pkg=@muse/recall · kind=compose · verdict=PASS · firesSinceDrill=2
ratchet: testFiles 1011 · fabrication 0 · groundedSurfaces 27 · commands-ask 2716->2691 LOC
- What: moved sufficiencyAdvisory (the set-level grounding-sufficiency advisory — when a multi-part query's notes cover only SOME clauses, emit an advisory-only "I found nothing on X" caveat; arXiv:2411.06037) out of the 2716-LOC commands-ask.ts CLI god-file into @muse/recall/verdict.ts. The function is pure grounding-presentation logic that belongs in the recall package next to drawBestGroundedRedraft/groundingVerdictNotice; verdict.ts ALREADY imports answerIsRefusal (./text.js) + from @muse/agent-core, so it only needed assessContextSufficiency added to its agent-core import (recall already depends on agent-core — no new dep, no cycle, no pnpm-lock change). commands-ask.ts: removed the fn def + its now-unused assessContextSufficiency import (it was the ONLY user), added sufficiencyAdvisory to the @muse/recall import + re-export so the call site (registerAskCommand:2546) and the existing 9-case cli test resolve unchanged. +4-case OUTCOME test at the recall home (packages/recall/test/sufficiency-advisory.test.ts: fires on partial cover, silent when fully covered, json/refusal/single-intent gates, fail-open on length mismatch).
- Why: compose KIND (0/8 in the last-8 window — best possible diversity; decompose was 5/8, the streak the ratchet's spirit warns against) AND the #1 value thread (Phase 3 @muse/recall extraction — consolidating grounding-presentation behind the recall seam). A SMALL verified step toward Phase 3 (a single pure helper) rather than the risky full pipeline+API extraction. (recall,compose) pair 0/8→1/8; pkg @muse/recall fresh in the window.
- Review point: ④b judge — sufficiencyAdvisory moved BYTE-IDENTICAL (same gates: json/refusal/multi-part/length-match + assessContextSufficiency + the exact advisory string); verify assessContextSufficiency was ONLY used by this fn in commands-ask (so dropping its import is safe — build proves it), answerIsRefusal STILL imported (used elsewhere in commands-ask, e.g. 2176/2357/2426); the re-export keeps the cli test (9 cases) + call site unchanged (behavior-preserving); recall build + 40 test files green; full check rc=0; no grounding-FLOOR change (the advisory is advisory-only, never touches the citation gate — moving it doesn't alter when/whether it fires).
- Risk: low — pure relocation of a self-contained grounding-presentation helper to its natural package home behind import+re-export; both the existing cli test (re-export path) and a new recall test (canonical home) green; deps already present in the target (no new dep/cycle/lockfile change); advisory-only so the grounding floor is untouched.

## fire 92 · 2026-06-14 · loop-creator v1.14.0 · f28ef6c8
meta: value-class=refactor · pkg=@muse/cli · kind=decompose · verdict=PASS · firesSinceDrill=3
ratchet: testFiles 1013 · fabrication 0 · groundedSurfaces 27 · commands-ask 2691->2676 LOC
- What: extracted parseBoundedInt (the strict bounded-integer parser for CLI numeric flags — undefined/blank → fallback; non-numeric / unit-slip / below-min → a flag-named throw rather than a silent default; valid → trunc+clamp to [min,max]) out of the 2691-LOC commands-ask.ts god-file into a new focused sibling apps/cli/src/parse-bounded-int.ts. The smell: a fully general validation util lived in the ASK command's file yet was imported by EIGHT unrelated commands (listen/orchestrate/routine/debug/runs/maintenance/inbox) + 2 tests — so every one of them transitively coupled to the 2676-LOC ask module's graph for one parser. commands-ask now imports it back for its 3 internal uses (--top/--calendar-days/--best-of) AND re-exports it, so all 8 consumers + tests resolve UNCHANGED. +4-case OUTCOME test (parse-bounded-int.test.ts: fallback on blank, parse-in-range, trunc+clamp-high, throw on unit-slip/non-numeric/below-min with the flag-named message).
- Why: high-value god-file reduction on the largest remaining cli file (commands-ask 2691 LOC) — and the extracted unit is the worst-coupled helper (8 importers). decompose KIND is 4/8 in the last-8 window but the gate is the PAIR: (cli,decompose) 2/8→3/8, well under 6/8; max pair in-window stays ≤3/8. Chose re-export (contract-preferred "호출부 무변경") over repointing the 8 sites this fire — recorded that decouple as a follow-up ◦. (commands-doctor is already decomposed to 674 LOC, so backlog:177 is stale.)
- Review point: ④b judge — parseBoundedInt moved BYTE-IDENTICAL (same blank-fallback, the `!Number.isFinite(parsed) || parsed < min` reject, the exact `[min, max] (got 'raw')` message, the `Math.min(max, Math.trunc(parsed))` clamp); a module-level WHY header was ADDED (the strictness is deliberate — a unit-slip is a loud error not a silent value — load-bearing, mirrors code-style.md's strict-Number example), NOT a per-line WHAT comment; import+re-export keeps the 8 consumers + commands-ask.test.ts (re-export path) green; cli build + 233 test files + check rc=0 + eslint 0; no behavior change.
- Risk: low — pure relocation of a self-contained pure function behind import+re-export; both a new dedicated OUTCOME test and the existing ask test (via re-export) green; no new dep / no lockfile change; the only addition beyond the move is a concise WHY header on the new module.

## fire 93 · 2026-06-14 · loop-creator v1.14.0 · 283ca0a2
meta: value-class=refactor · pkg=@muse/mcp · kind=decompose · verdict=PASS · firesSinceDrill=4
ratchet: testFiles 1014 · fabrication 0 · groundedSurfaces 27 · loopback-relative-time 914->650 LOC
- What: split the 914-LOC loopback-relative-time.ts god-file (the reminder/calendar date-phrase resolver) by LANGUAGE. Extracted the entire Korean cluster — 2 lookup-table consts (KOREAN_DAY_OFFSET, KOREAN_WEEKDAY_ISO) + 4 file-private fns (resolveKoreanRelativePhrase entry + resolveKoreanWeekdayPhrase/resolveKoreanDurationOffset/parseKoreanTimeOfDay internals), 245 lines — into a new sibling loopback-relative-time-korean.ts. Because the Korean cluster and the English resolver BOTH use addCalendarMonths/startOfDay/DEFAULT_HOUR/DEFAULT_MINUTE (and the English resolver calls into Korean), a naive extraction would cycle; so a third module loopback-relative-time-base.ts holds those 4 shared date primitives, and the graph is acyclic: main→base, main→korean→base. main 914→650 LOC. resolveRelativeTimePhrase now imports resolveKoreanRelativePhrase + the base primitives; its delegation (finiteDate(resolveKoreanRelativePhrase(...))) is unchanged. +9-case OUTCOME test (loopback-relative-time-korean.test.ts: base addCalendarMonths month-overflow clamp + rollover, startOfDay zeroing; Korean entry resolves 내일/3일 뒤, undefined for non-Korean).
- Why: highest-value clean slice available — the largest real-logic god-file in a COLD package (mcp; only followup-firing-loop.ts is dirty there), split on the natural English/Korean seam. decompose KIND is 4/8 in-window but on a FRESH pair (mcp,decompose)=0/8→1/8; max in-window pair stays (cli,decompose)=3/8, well under 6/8. Non-decompose alternatives this fire were thin (only 1 genuine orphaned comment in commands-ask; commands-doctor already decomposed to 674 LOC). Phase-3 recall compose deferred (the clean recall helpers are extracted; the remaining is the design-sensitive pipeline).
- Review point: ④b judge — the Korean cluster + base primitives moved BYTE-IDENTICAL (sed range-extraction, only `function`→`export function` on the 3 cross-module entries + a removed no-op comment on the DEFAULT consts); the cycle is genuinely broken (verify main→korean→base is acyclic, no korean→main edge); BUILD initially caught a missed const dep (DEFAULT_HOUR/MINUTE — shared, moved to base) — fixed, second build clean; resolveRelativeTimePhrase delegation unchanged so all Korean phrase behavior is preserved (mcp.test.ts + relative-time-period.test.ts = 431+47 green); the 3 inner Korean fns stay file-private in korean.ts (only the entry is exported); no @muse/mcp index/barrel change, no new package dep (intra-package), no lockfile change.
- Risk: low-moderate — a 3-way split of a 914-LOC file is more surface than a single move, but every moved line is byte-identical (sed-extracted) and the behavior is pinned by 478 existing relative-time assertions + 9 new ones; the one real hazard (a cycle, or a missed shared symbol) was caught by tsc and resolved (DEFAULT_HOUR/MINUTE → base). No behavior change, no public API change.

## fire 94 · 2026-06-14 · loop-creator v1.14.0 · 72357c60
meta: value-class=refactor · pkg=@muse/cli · kind=dead-code · verdict=PASS · firesSinceDrill=5
ratchet: testFiles 1015 · fabrication 0 · groundedSurfaces 27 · knip program.ts dead re-exports 4->0
- What: removed 4 dead re-exports through apps/cli/src/program.ts — `defaultCredentialPath` (from credential-store.js), `writeRunLog` (from program-helpers.js), and `appendActivity`+`maybeCompactLastChatHistory` (from chat-history.js) — plus the 3 stale back-compat comment blocks that explained them. These were historical "keep ./program.js import paths working" re-exports whose consumers have since moved to importing from the source modules directly. Verified dead via knip + a repo-wide grep + an apps/cli/test/ check: NO file imports any of the 4 through program.js (the comment claiming program.test.ts imports defaultCredentialPath from program.js is stale — that test imports createProgram/defaultConfigPath/uniqueCommandPrefix, and dynamically imports maybeCompactLastChatHistory from chat-history.js at line 8627). Kept `readPipedStdin` (knip did NOT flag it → still consumed through program.js) and `writeRunLog`'s IMPORT at line 33 (used internally at program.ts:434).
- Why: MUST vary KIND off decompose (5/8 in the last-8 window — the gate is per-pair but the spirit warns against KIND monotony). dead-code is 1/8. Chose program.ts's dead re-exports (knip-objective, fire-87/89-style verified pattern) over the isNodeError "dedup" (which is NOT a clean cohere — the 3 copies have DIFFERENT impls: chat-history uses isRecord+string-code, the other two use `instanceof Error`, so consolidating would change behavior). (cli,dead-code) pair 1/8→2/8.
- Review point: ④b judge — all 4 are genuinely dead (knip + grep + apps/cli/test/ confirm no via-program.js import); the removed comments were stale historical narration (the defaultCredentialPath one was provably wrong), NOT load-bearing WHY; readPipedStdin's re-export + writeRunLog's internal import are intact (cli build + eslint 0 prove no orphaned import); program.test.ts (236) + full cli suite (233 files) green; knip re-scan shows the 4 gone; no behavior change (re-exports are pure surface, every consumer imports from source).
- Risk: low — removal of 4 unused re-export surfaces + stale comments; knip-objective + grep + test-dir verified; no new module/test (removal slice, like fire 87); the one trap (a test importing through program.js) was checked and refuted; behavior untouched.

## fire 95 · 2026-06-14 · loop-creator v1.14.0 · 09161a36
meta: value-class=refactor · pkg=@muse/memory · kind=decompose · verdict=PASS · firesSinceDrill=6
ratchet: testFiles 1016 · fabrication 0 · groundedSurfaces 27 · memory-auto-extract 696->588 LOC
- What: extracted the input-sanitization cluster from the 696-LOC memory-auto-extract.ts into a new sibling memory-auto-extract-sanitize.ts (119 LOC). Moved: the ExtractedSlot interface + sanitizeSlotArray + sanitizeEntries (the two entry points called by persist()) + sanitizeValue + normalizeKey (cluster-internal). This is the defensive boundary between an extractor model's raw output and UserMemoryStore — count/key/value caps, key normalization (lowercase/[^a-z0-9_]→_), the typeof-[]==='object' array-footgun reject, and terminal-control-byte stripping (anti-memory-poisoning). Its only non-builtin dep is stripUntrustedTerminalChars (@muse/shared, which memory already depends on), so the move is acyclic (sanitize→@muse/shared only; main→sanitize). main imports {sanitizeEntries, sanitizeSlotArray, type ExtractedSlot} back; ExtractionPayload still names ExtractedSlot via the import. +6-case direct OUTCOME test at the new home (the existing auto-extract-sanitize.test.ts covers it INDIRECTLY through the hook and still passes — behavior preserved).
- Why: package diversity — cli was 5/8 of the last-8 window, so this fire goes to a FRESH package (@muse/memory, never touched by codebase-quality before); (memory,decompose) is a fresh pair 0/8, max in-window pair stays (cli,decompose)=3/8 < 6/8. memory-auto-extract is the largest memory file (696 LOC) and the sanitization is a self-contained, security-relevant unit that benefits from an isolated, directly-testable home (it was only hook-tested before). decompose KIND is 5/8 but the gate is per-pair.
- Review point: ④b judge — the cluster moved BYTE-IDENTICAL incl. the load-bearing WHY comments (the array-footgun explanation, the terminal-strip rationale) — verify by diff; only sanitizeSlotArray/sanitizeEntries got `export` (sanitizeValue/normalizeKey stay file-private — confirm nothing outside the cluster called them); ExtractedSlot moved to the sanitize module (its validation owner) + main imports it for ExtractionPayload (acyclic, no main→… cycle); stripUntrustedTerminalChars still used in main (line ~269) so its @muse/shared import stays (eslint 0 proves no orphan); memory build + 37 test files green; the indirect hook test (auto-extract-sanitize.test.ts) unchanged + passing = behavior preserved.
- Risk: low — relocation of a self-contained pure-validation cluster behind import; byte-identical + a new 6-case direct test + the existing indirect hook test both green; one cross-package dep (stripUntrustedTerminalChars) already present in the target; no new dep / no lockfile change / no @muse/memory index change (the sibling is imported directly, not re-exported from the barrel).

## fire 96 · 2026-06-14 · loop-creator v1.14.0 · e94e5694
meta: value-class=refactor · pkg=@muse/recall+mcp+cli · kind=comment-hygiene · verdict=PASS · firesSinceDrill=7
ratchet: testFiles 1016 · fabrication 0 · groundedSurfaces 27 · forbidden-marker comments 4->0 (safe pkgs)
- What: a comment-hygiene batch removing the last forbidden goal-marker + orphan-comment rot in non-hot packages (code-style.md: round/goal/Pnn-nn refs are "delete on sight"). (a) recall/select.ts — stripped the `(P37-20)` / `(P37-21)` goal-ref parentheticals from the grounding-evidence doc, KEEPING the load-bearing field list (relationship/role, connections/edges, email/phone/...). (b) mcp/personal-playbook-store.ts — stripped the `P43-1` marker, KEEPING the encryption-at-rest WHY ("the capstone of the self-learning safety: Muse learns you in the background AND the learned model can't leak"). (c) commands-ask.ts — removed the orphaned `/** Prepend the ACE [Learned Strategies] block ... */` doc comment that documented a function relocated long ago (a JSDoc block followed by a blank line then ANOTHER doc block — documents nothing; the fire-92 review finding).
- Why: vary KIND HARD off decompose (5/8 in the last-8 window — the gate is per-pair but the spirit warns against KIND monotony, and the recent journals kept deferring this). comment-hygiene is 0/8 — best possible diversity. The forbidden-marker vein is thin (a comprehensive scan found only these 3 sites + 3 "fire 9" refs in HOT agent-core, deliberately left, + the "round 1" DOMAIN term in commands-swarm which is load-bearing). This exhausts the safe-package marker rot. pkg spread recall/mcp/cli (none cli-decompose).
- Review point: ④b judge — every edit is COMMENT-ONLY (zero runtime/AST-emit change — verify the diff touches only ` * ...` / `//` lines and the removed commands-ask block had NO associated declaration); the stripped tokens (P37-20/P37-21/P43-1) are pure goal-refs (forbidden), NOT load-bearing WHY — the field lists + encryption rationale were preserved; the orphan genuinely documented nothing (next non-blank line was another `/**`); recall/mcp/cli build clean + full check rc=0 + lint 0 + self-eval groundedSurfaces 27; no behavior, no test delta (comment-only, no new module).
- Risk: minimal — comment-only removals of code-style.md-forbidden markers + one verified-orphan doc block; load-bearing WHY explicitly preserved in each; no logic touched; behavior identical by construction.

## fire 97 · 2026-06-14 · loop-creator v1.14.0 · f3cfb841
meta: value-class=refactor · pkg=@muse/cli · kind=dead-code · verdict=PASS · firesSinceDrill=0 (reset — 7th JUDGE-DRILL completed)
ratchet: testFiles 1017 · fabrication 0 · groundedSurfaces 27 · knip commands-doctor dead re-exports 2->0
- What: TWO parts. (1) 7th JUDGE-DRILL (8 consecutive allPASS trigger): injected a comment-hygiene-looking commit "trim verbose comments in the sanitize module" that DELETED two load-bearing SECURITY-WHY comments from memory-auto-extract-sanitize.ts — the `typeof [] === "object"` array-footgun explanation (why sanitizeEntries rejects arrays: a wrong-shape LLM payload would silently land fake "0"/"1" keys in UserMemoryStore) and the sanitizeValue store-boundary rationale (why control-byte stripping happens here: anti-prompt-injection / anti-terminal-hijack). It passed ALL deterministic gates (build clean, 47 auto-extract tests green, eslint 0 — comments don't affect them). The independent Opus ④b judge correctly FAILed it, identifying both as KEEP-category load-bearing WHY (non-obvious security invariants a reader can't derive from the code) and flagging the misleading "self-documenting / no behavior change" framing + the un-applied "condense to one WHY line" remedy. Rolled back via `git reset --hard HEAD~1` (both comments restored). (2) REAL slice: removed 2 dead re-exports through commands-doctor.ts — `export type { CalibrationReport }` (line 17) + `readOllamaPerfEnv` from the checks re-export (line 26). Both knip-flagged; verified no consumer imports them via the commands-doctor barrel (program.ts pulls only registerDoctorCommand; commands-doctor.test.ts pulls buildCalibrationReport/formatCalibration). The underlying defs stay (CalibrationReport needed for formatCalibration's declaration; readOllamaPerfEnv imported at line 18 + used at line 328 — that import/use is untouched).
- Why: JUDGE-DRILL was DUE (8 consecutive allPASS, fires 89-96 — hard-counter, not deferrable). The drill picked a NEW axis (load-bearing-WHY deletion, the exact risk of the comment-hygiene KIND that fire 96 exercised — gates-invisible, only judgment catches it). Real slice = dead-code (1/8 — varies off decompose 4/8) on the fire-90-scouted commands-doctor re-exports; (cli,dead-code) pair 1/8→2/8, max in-window pair stays ≤2/8.
- Review point: ④b judge — (drill) confirm the judge's FAIL reasoning was sound (both deleted comments ARE non-obvious security WHY, not WHAT-narration) and the rollback restored them verbatim; (real) both removed re-exports are genuinely dead (knip + grep + test-dir confirm no via-barrel import); the line-18 import + line-328 use of readOllamaPerfEnv are intact (cli build + eslint 0 prove no orphan); commands-doctor.test.ts (75) + full cli suite (233 files) green; knip re-scan shows the 2 gone.
- Risk: low — the drill never touched main (branch commit → judge FAIL → reset); the real slice is a removal of 2 unused re-export surfaces, knip+grep+test verified, no behavior change, no new module/test. The drill confirms the maker≠judge verifier catches a gates-invisible load-bearing-WHY deletion in a security module.

## fire 98 · 2026-06-14 · loop-creator v1.14.0 · 022d814a
meta: value-class=refactor · pkg=@muse/api · kind=dead-code · verdict=PASS · firesSinceDrill=1
ratchet: testFiles 1017 · fabrication 0 · groundedSurfaces 27 · knip currentAuthIdentity export removed
- What: de-exported `currentAuthIdentity` in apps/api/src/compat-user-memory-store.ts. knip flagged it as an unused EXPORT — it's used ONLY internally (line 103 of the same file, inside the bearer-identity compatibility check) and has NO external importer (the admin test references it only in a comment). Narrowed `export async function` → `async function`. fresh package (@muse/api — first codebase-quality fire here). EARLIER THIS FIRE: attempted an embedder COHERE (move createOllamaEmbedder + createGateEmbedder from context-engineering-builders.ts into embedder-base.ts) — built clean, +4 new embedder-factory tests green, but `pnpm self-eval` flagged a tracked-count REGRESSION (egressGuards 7→6): self-eval.mjs's egressSources hardcodes context-engineering-builders.ts and counts the `throw new LocalOnlyViolationError(` there, so relocating the guard (still firing — behavior preserved, the test proved it) dropped the count. Per the hard rule (don't weaken/edit the honesty machinery; self-eval is human-directed infra) I REVERTED rather than edit egressSources, and recorded a backlog BLOCKER.
- Why: the embedder cohere was the value-first pick (consolidates embedder construction, reunites a separated doc, removes 4 stale imports) but its clean form requires a human to update the egressGuards source list — out of loop scope. Fell back to a clean, safe dead-code slice that touches NO egress-tracked file. dead-code is 2/8 in the last-8 window; @muse/api is a fresh package (cli was overrepresented). (api,dead-code) pair 0/8.
- Review point: ④b judge — currentAuthIdentity is genuinely internal-only (knip + grep: used at line 103, defined at 107, zero external imports; test mention is a comment); de-export is a pure visibility narrowing (build proves the internal call still resolves; eslint 0 — not now-unused since it IS used internally); api build + 145 test files + full check rc=0; knip no longer flags it; egressGuards stays 7 (this slice touches no egress file). NOTE: the embedder cohere was reverted (working tree clean before this commit) — only compat-user-memory-store.ts + docs are in the diff.
- Risk: low — a single export-visibility narrowing of an internal-only function; knip+grep+build verified; no behavior change; the riskier embedder cohere was correctly reverted on the egressGuards regression (the ratchet did its job — caught a guard relocation the counter couldn't follow).

## fire 99 · 2026-06-14 · loop-creator v1.14.0 · ca6f3528
meta: value-class=refactor · pkg=@muse/agent-core · kind=cohere · verdict=PASS · firesSinceDrill=2
ratchet: testFiles 1019 · fabrication 0 · groundedSurfaces 27 · median dup 3->1 source
- What: consolidated three BYTE-IDENTICAL file-private `median(sortedAscending: readonly number[]): number` copies — in relationship-decay.ts (contact-cadence median), activity-anomaly.ts and change-point.ts (both the modified-z-score median + the MAD-scale median) — into a new shared packages/agent-core/src/median.ts. Each detector now imports `median` from "./median.js" instead of hand-rolling its own. The 3 copies were verified identical via diff and each is file-private; every call site already pre-sorts (`median([...x].sort((a,b)=>a-b))`), so the "caller sorts, this is an O(1) pick" contract is preserved verbatim. +4-case OUTCOME test (empty→0, odd→middle, even→avg-of-two-middle, and a pin on the assumes-sorted contract). The related `medianGap` in @muse/mcp (note-family-absence/personal-episodes-store) is a DIFFERENT function (takes unsorted input, sorts internally) — NOT consolidated.
- Why: cohere is 0/8 in the last-8 window — the best KIND diversity (dead-code was 3/8, decompose 3/8). A genuine clean duplication (3 identical pure copies) is the textbook cohere; consolidating gives the median one tested home and removes 2 copies. The other "duplicate" candidates were rejected as NOT byte-identical (formatBytes — 4 different unit/precision impls; isNodeError — instanceof vs isRecord; errorBody — different error classes), exactly the trap a real cohere must avoid. pkg @muse/agent-core (the median copies live there); (agent-core,cohere) pair fresh.
- Review point: ④b judge — the moved median body is BYTE-IDENTICAL to all 3 originals (diff-verified) and the new module's only addition is a load-bearing WHY doc (the pre-sorted contract — non-obvious from the name); each of the 3 files dropped its def + added the import (no leftover dup); each caller still sorts before calling so behavior is identical; agent-core build + the 3 detector test files (relationship-decay/activity-anomaly/change-point) + median.test.ts all green (23); full check rc=0 (agent-core 168 files); median.ts is package-internal (NOT added to the agent-core barrel). HOT-PACKAGE NOTE: agent-core is the concurrent cognition loop's package — if the merge conflicts on one of the 3 files, defer per contract.
- Risk: low-moderate — pure consolidation of 3 identical pure-function copies behind an import; byte-identical + a new 4-case test + the 3 detectors' existing tests all green; no new dep (intra-package); the only elevated risk is a merge conflict with the concurrent agent-core loop (handled by defer-on-conflict, the branch commit is safe).

## fire 100 · 2026-06-14 · loop-creator v1.14.0 · 9ca019f4
meta: value-class=refactor · pkg=@muse/mcp · kind=cohere · verdict=PASS · firesSinceDrill=3
ratchet: testFiles 1020 · fabrication 0 · groundedSurfaces 27 · medianGap dup 2->1 source
- What: consolidated the 2 BYTE-IDENTICAL file-private `medianGap(values: readonly number[]): number` copies — in note-family-absence.ts (note-folder cadence baseline) and personal-episodes-store.ts (episode-series cadence) — into a new shared packages/mcp/src/median-gap.ts. Both cadence detectors now import it. The 2 copies were diff-verified identical and each called exactly once (note-family-absence:79, personal-episodes-store:414). medianGap KEEPS its internal sort (`[...values].sort()` then median-of-sorted, empty⇒0) because @muse/mcp does NOT depend on @muse/agent-core — so it can't reuse fire-99's `median(sortedAscending)` (which is also package-internal, not barrelled). +4-case OUTCOME test (empty→0, sorts-unsorted-input [median([9,1,5])===5 not the positional 1], even-average, outlier-robust). This is the mcp sibling of fire-99's agent-core median cohere.
- Why: cohere is the best available KIND (compose 0/8 was the target but no clean recall compose exists — see below; dead-code 3/8 + decompose 3/8 are saturated). A genuine clean duplication (2 identical pure copies) is the textbook cohere. RECALL-COMPOSE REJECTED this fire: relevantExcerpt (commands-recall.ts) looked like a clean move into @muse/recall (next to relevantSnippet) but it calls a cli-LOCAL lexicalOverlap (recallContentTokens-tokenized, returns a normalized FRACTION) that DIFFERS from @muse/agent-core's lexicalOverlap (lexicalTokens, raw COUNT) which present.ts uses — moving it would silently change its best-line selection. (mcp,cohere) pair fresh in window; both target files were clean in the primary worktree (low conflict risk).
- Review point: ④b judge — the moved medianGap body is BYTE-IDENTICAL to both originals (diff-verified) + the new module's only addition is a load-bearing WHY doc (sorts-internally + empty⇒0 + the median-vs-mean robustness rationale); each detector dropped its def + added the import (no leftover dup); the 2 call sites are unchanged (`medianGap(gaps)`); mcp build + the 2 detector tests + median-gap.test.ts green; full check rc=0 (mcp 196 files); median-gap.ts is package-internal (not barrelled). NOTE: medianGap is NOT the same function as agent-core's median — it sorts internally; consolidating them across packages was correctly NOT done (would need a new mcp→agent-core dep).
- Risk: low — consolidation of 2 identical pure-function copies behind an import; byte-identical + a new 4-case test + the 2 detectors' existing tests green; no new dep (intra-package); both target files clean at sync (low merge-conflict risk).

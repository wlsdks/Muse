...
meta: value-class=correctness-bug(memory-key 한글 드롭, 2번째 진입점) · pkg=apps/cli · kind=/remember·/pref parseRememberArg Unicode 키 보존 · verdict=PASS · firesSinceDrill=9
ratchet: testFiles 1009 유지(기존 parseRememberArg 블록에 +2 케이스) · fabrication 0 유지 · eval 무변동(슬래시-커맨드 파서, MuseTool 아님) · 패키지 다양화 @muse/mcp→apps/cli
- **무엇:** fire-128 scout가 기록한 ◦ 구현: `/remember key=value`(+`/pref`)의 `parseRememberArg`가 키를 ASCII `[^a-z0-9_]`로 slug → '/remember 취미=등산'이 ""→undefined→"Tell me what to remember…" 무기록. fire 128 remember_fact 버그의 **2번째 쓰기 진입점**(인터랙티브 CLI 경로 — 진안이 직접 타이핑). `normalizeMemoryKey`(@muse/memory) 위임 + `/[\p{L}\p{N}]/u` 가드.
- **왜:** 진안(KO)이 채팅에서 한글로 /remember를 치면 조용히 데이터 손실 — fire 128(remember_fact MuseTool)과 같은 root-cause 클래스지만 다른 진입점·다른 패키지(apps/cli). 두 surface 모두 고쳐야 완결(model 도구 경로 + 인터랙티브 CLI 경로). normalizeMemoryKey로 /forget·/memory와 라운드트립 일관성 확보. VALUE-CLASS RATCHET: @muse/mcp→apps/cli 패키지 다양화(같은 KIND 2연속이나 다른 패키지·유저 직격 correctness라 padding 아님).
- **리뷰지점:** RED 확인(취미=등산→undefined) → fix → 231 cli test 파일 green·내 2파일 lint clean. ④b Opus judge PASS 7/7 — #2(ASCII 기대값 city/reply_style/home_town 무변동을 normalizeMemoryKey 독립 계산으로 검증, fire 128 "Favorite-Drink!" 같은 divergence 없음)·#3(가드 load-bearing: normalizeMemoryKey("!!!")=="!!!"이라 "!!!=value" 거부에 가드 필수)·#4(/forget·/memory·remember_fact와 동일 normalizer라 라운드트립 일관)·#6(유저 직격 데이터손실, padding 아님) 무자비 검증. pnpm check의 agent-core followup-capture-hook.test.ts FAIL은 동시 cognition 루프의 미커밋 user_context 기능(agent-core는 apps/cli 의존 안 함 → 내 변경 무영향).
- **리스크:** 없음 — 순수 파서, IO 무, 무효입력 무기록 불변식 유지, 의존성(@muse/memory)은 이미 선언+import(무순환). LESSON: 같은 root-cause 버그는 모든 진입점을 audit(model 도구 경로 ≠ 인터랙티브 CLI 경로 — 둘 다 같은 ASCII slug 카피). memory-key 한글 드롭 클래스 이제 양쪽 진입점 COMPLETE. 다음 fire 130 = JUDGE-DRILL(firesSinceDrill≥10). pnpm check red는 동시 루프 noise.


## fire 130 · 2026-06-14 · skill v1.14.0 · 379a61f4
meta: value-class=correctness-bug(weather 불가능-날짜 echo) + JUDGE-DRILL · pkg=@muse/mcp · kind=weather resolveTargetDateIso 캘린더 유효성 검증 · verdict=PASS · firesSinceDrill=0(드릴 완료, 리셋)
ratchet: testFiles 1010 유지(weather-tool.test +2 케이스) · fabrication 0 유지 · eval 무변동(execute-only, 선택/스키마 무변경) · 패키지 apps/cli→@muse/mcp
- **무엇:** [JUDGE-DRILL] firesSinceDrill 10 도달 → 고의 나쁜-슬라이스 주입(unit_convert "hardening" = JSDoc 리워드 + 선언-only 테스트 3개[name/risk/typeof·description-contains-area·이미-커버된 km→m]) → 결정적 게이트는 통과(빌드/13파일 green/lint) → ④b Opus judge가 **FAIL**(선언-only·duplicate-coverage·cosmetic, 기존 muse-tools-units.test가 이미 전부 커버 인용) → git restore+rm 롤백. maker≠judge 롤백 경로 재검증 완료. [REAL FIX] weather 도구 `resolveTargetDateIso`가 `\d{4}-\d{2}-\d{2}` 모양만 보고 유효성 검증 없이 echo → "2026-02-30"/"2026-13-45"(12B 날짜-산술 실수)을 `date:"2026-02-30", reason:"no forecast for that day(out of range)"`로 반환 = 불가능 날짜를 "범위 밖 실재 날짜"로 모델에 단언(grounded-lie hook). `isValidCalendarDate`(Date.UTC round-trip) 추가 → 무효 날짜는 기존 "couldn't understand the day" 정직 경로로, provider 호출 안 함.
- **왜:** [드릴] firesSinceDrill≥10 하드-카운터 트리거 — 미루기 불가, 검증자가 여전히 선언-only/padding을 잡는지 적대 재확인(이번엔 "coverage-theater" 클래스, fire 110/120의 inert-tool과 다른 변종). [실수정] fabrication=0 floor 정렬: 도구의 구조화 출력이 불가능 날짜를 실재 날짜로 확인해주면 모델이 "2월 30일엔 예보 없음"식으로 가짜 날짜를 강화 → 정직한 거부가 grounding-aligned correctness. 패키지 다양화(apps/cli→mcp).
- **리뷰지점:** [드릴] 결정적 게이트 통과하나 judge가 4/4 criteria FAIL(기존 동일-날 comprehensive 테스트 인용) → 롤백 검증. [실수정] RED 확인(2026-13-45 echoed) → fix → 193 mcp green·lint clean. ④b judge(별개 fresh instance) PASS 5/5 — mutation-check(가드 되돌리면 RED)·validator 독립검증(2024-02-29 leap accept, Feb30/month13/month0 reject, 4자리 연도 무오거부)·full-ISO prefix("2026-05-31T15:00Z"→"2026-05-31") 보존·relative-phrase 경로 무영향 무자비 검증. honest: value는 real but modest(저빈도 fail-soft)이나 grounded-lie hook 제거라 padding 아님.
- **리스크:** 없음 — execute-only(선택/스키마 무변경 → eval:tools 골든 불요), 무효 날짜만 거부(유효·leap·ISO-prefix 보존), relative 경로 무관, 순수 검증 함수. LESSON: JUDGE-DRILL은 변종을 돌려야(inert-tool→cosmetic→coverage-theater) 검증자의 discrimination을 넓게 확인. 검증자가 "결정적 게이트 통과 ≠ 가치"를 일관 적용함을 재확인. pnpm check의 agent-core followup-capture-hook.test.ts FAIL은 동시 cognition 루프 미커밋 user_context 기능(내 변경 무관). 다음: 드릴 리셋 후 일반 슬라이스.


## fire 131 · 2026-06-14 · skill v1.14.0 · 4776ea7b
meta: value-class=new-capability(EXPANSION) · pkg=@muse/tools(+scripts eval) · kind=korean_number 한국 만/억/조 숫자 그룹핑 도구 · verdict=PASS · firesSinceDrill=1
ratchet: testFiles 1010→1011(+muse-tools-korean-number.test) · fabrication 0 유지 · eval:tools +1 시나리오(korean-number 6 케이스 STABLE 3/3) · value-class correctness-bug 3연속→EXPANSION 다양화
- **무엇:** 새 EXPANSION `korean_number`(아라비아 정수 → 한국 만/억/조 그룹핑). 12B는 서구식 3자리 콤마로 그룹핑해 만/억 경계를 틀림(12345678 → "1234만 5678"가 정답). 4자리 청크, zero-chunk 생략(100000005→"1억 5", "1억 5만"=100050000과 비충돌), 음수, 경 초과→error. createMuseTools 배선(21 도구). digits→Korean only, 역방향(Korean→digits) follow-up ◦.
- **왜:** 3연속 correctness-bug(128 remember_fact·129 /remember·130 weather) + actuator/MCP correctness vein 얇아짐 → value-class를 EXPANSION으로 올림(EXHAUSTION 가이드: scout clean이면 value-class 상승). 평/lunar와 같은 user-specific 결정적 grounding(진안 Korean, 모델 unreliable). 결정적이라 grounded-lie 위험 0(알고리즘 정확성만 확보하면).
- **리뷰지점:** 알고리즘 node 선검증(11 케이스) → RED(모듈 부재) → impl → 13 tools 파일 green. eval:tools korean-number STABLE 3/3 ×6(만/억 selection 3 + value arg + unit_convert/math_eval carve 2 + 숫자-포함 musing IrrelAcc). ④b Opus judge PASS 5/5 — #2(알고리즘 독립검증: 10 spec + edge 99999999→"9999만 9999"·10001→"1만 1"·non-int→undefined·1경→"1경"·out-of-range→undefined, zero-chunk 무충돌 "1억 5"≠"1억 5만")·#1(배선 21 inventory)·#3(평/lunar-class value, padding 아님)·#4(name 비충돌·use-when/not-when·rich schema) 무자비 검증. nit: MAX_SAFE_INTEGER 초과(>9천조 원, 비현실) float-round — judge "실무상 무관". pnpm check agent-core FAIL은 동시 루프 user_context(무관).
- **리스크:** 없음 — 순수 read 도구, 결정적 변환(알고리즘 judge 독립검증), bad input→error(throw 안 함), 기존 20 도구 무영향. LESSON: correctness-bug 3연속 후 EXPANSION으로 value-class 다양화(KIND 다양성만으론 단조 못 막음). digits→Korean 후 reverse는 lunar_date→lunar_to_solar처럼 follow-up. 다음: reverse 또는 다른 KIND.


## fire 132 · 2026-06-14 · skill v1.14.0 · 37b16836
meta: value-class=new-capability(EXPANSION, korean_number 양방향 완성) · pkg=@muse/tools(+scripts eval) · kind=korean_number 역방향(한국어→정수) 파서 · verdict=PASS · firesSinceDrill=2
ratchet: testFiles 1012 유지(korean-number.test +역방향 케이스) · fabrication 0 유지 · eval:tools korean-number 6→8 케이스(STABLE 3/3) · 양방향 완성
- **무엇:** fire-131 follow-up ◦ 구현: korean_number를 양방향으로 확장(같은 도구, 혼동 sibling 아님). 새 `fromKoreanNumber`가 한국 만/억/조 식을 정수로 파싱 — digit 청크("1234만 5678"), 천/백/십 sub-unit + 복합("천만"=10⁷, "2천만"=2×10⁷), 후행 원, 콤마. execute가 방향 자동감지([조억만천백십] 포함 string→역방향, 아니면 number→정방향), 항상 {value, korean} 양형 반환. 스키마 `value`를 Gemini-safe 단일 string으로(union type 회피).
- **왜:** 역방향이 comprehension 측 고가치(개인 AI엔 타이핑된 "1억 2천만"→120000000 이해가 생성보다 중요), 8B는 복합 만/억 파싱을 틀림. lunar_date→lunar_to_solar처럼 양방향 완성. 같은 도구 확장(혼동 sibling 회피)이 tool-calling 정답. EXPANSION value-class 연속이나 ratchet 무관(micro-fix 아님), 새 KIND(역방향 파서).
- **리뷰지점:** 알고리즘 node 선검증(16+ 케이스) → RED(fromKoreanNumber 부재, 5 fail) → impl → 13 파일 green. eval STABLE 3/3 ×8(정방향3+역방향2+unit_convert/math_eval carve2+IrrelAcc). ④b Opus judge PASS 5/5 — #2(역방향 파서 독립검증: 8 필수 + 복합 "2천만"=2e7·"백만"=1e6·"1억2천만" 무공백·"1조 2345억 6789만 1234"·round-trip property n≥10000 모두 정확, 악성입력 "만억"/"1억 2억"도 plausible-lie 아닌 방어적 누적)·#1(정방향 무회귀)·#3(방향 자동감지 무모호)·#4(comprehension 고가치) 무자비 검증. judge가 union-type Gemini cloud 잠재이슈 지적 → 선제적으로 string 단일타입으로 narrow + eval 재검증 8/8(judge가 narrow 권고함).
- **리스크:** 없음 — 순수 read, 결정적 파싱(judge 독립검증 + round-trip), bad input→error, 정방향 무회귀, 스키마 narrow로 cloud 호환까지 확보. LESSON: judge의 latent-caveat(local PASS여도 cloud union-type 위험)를 선제 수정해 provider-neutral 유지. 커밋 메시지 백틱이 zsh 명령치환 트리거(단어 1개 소실) — -m 메시지에 백틱 금지(heredoc -F 사용). 다음: korean_number 완결, 다른 KIND/패키지.


## fire 133 · 2026-06-14 · skill v1.14.0 · 011fcfac
meta: value-class=hardening(security fail-close, guard가 자기 fail-close 소유) · pkg=@muse/agent-core · kind=createLlmClassificationInputGuard 분류기-장애 fail-close · verdict=PASS · firesSinceDrill=3
ratchet: testFiles 1014 유지(guards.test +3 fail-close 케이스) · fabrication 0 유지 · eval 무변동(결정적 guard, MuseTool 아님) · 패키지 @muse/tools→@muse/agent-core 다양화
- **무엇:** LLM-분류 입력 guard(injection/credential-abuse/policy-bypass 차단)가 provider.generate+parse를 try/catch 없이 호출 → 분류기 outage(throw)나 unparseable/unknown-action verdict가 예외로 전파. 파이프라인 generic catch가 fail-close는 했으나 (a)raw provider 에러(내부 host/IP)를 GuardBlockedError reason+metrics+monitor에 leak (b)generic GUARD_ERROR 코드(guard 버그와 구분 불가) (c)fail-close가 파이프라인 catch에 의존(guard가 소유 안 함). 이제 guard가 catch→clean `{allowed:false, code:LLM_CLASSIFICATION_UNAVAILABLE, reason:"...failing closed"}` 반환.
- **왜:** CLAUDE.md "Guards are fail-close · Security is deterministic code" — 보안 guard는 자기 fail-close를 결정적으로 소유해야(파이프라인 refactor가 silent fail-open으로 뒤집을 위험 제거 + raw 에러 info-leak 제거 + observability 구분 코드). hardening 테마에서 "보안 작업 자체가 가치". 패키지 다양화(@muse/tools 5/8 연속→agent-core), 새 KIND(security guard, Korean-utility 아님).
- **리뷰지점:** 파이프라인이 throw를 이미 catch함을 먼저 확인(guard-pipeline.ts:50-66) → 진짜 가치는 "guard 소유 + info-leak 제거 + 구분 코드"임을 honest 판정. RED(3 fail-close 케이스가 throw로 fail) → try/catch impl → guards.test.ts 격리 green·lint clean. ④b Opus judge PASS 5/5 — crux #2(파이프라인이 이미 fail-close하는데 cosmetic 아닌가?)를 info-leak 실재 추적(error.message→metrics/monitor/reason, guard-pipeline.ts:58/64/65)으로 "real hardening, not cosmetic" 판정·#1(evaluate가 throw→clean return 행동 델타)·#3(전 에러경로 allowed:false, block은 try 밖이라 안 삼킴)·#4(allow/block 무회귀) 무자비 검증. eval:tools 불요(결정적 guard).
- **리스크:** 없음 — try가 generate+parse만 감싸고 decision 분기는 밖(정상 allow/block 무영향), 전 에러경로 fail-close(allowed:true 경로 없음), 순수 결정적. LESSON: "이미 fail-close"여도 guard가 자기 결정을 소유 vs 파이프라인 catch 의존은 다름(info-leak + silent-flip 위험). JUDGE-DRILL 후라 judge가 cosmetic 경계를 엄격 판정했으나 info-leak 실재로 PASS. pnpm check agent-core user_context FAIL은 동시 루프(무관). 다음: 다른 KIND/패키지.


## fire 134 · 2026-06-14 · skill v1.14.0 · 2e5c82a5
meta: value-class=new-capability(EXPANSION) · pkg=@muse/tools(+scripts eval) · kind=epoch_convert 유닉스 타임스탬프↔날짜 도구 · verdict=PASS · firesSinceDrill=4
ratchet: testFiles 1014→1015(+muse-tools-epoch.test) · fabrication 0 유지 · eval:tools +1 시나리오(epoch-convert 6 케이스 STABLE 3/3) · 새 KIND(epoch, Korean-utility 벗어남)
- **무엇:** 새 EXPANSION `epoch_convert`(유닉스 타임스탬프↔달력 날짜, 양방향). 숫자→UTC ISO(sec/ms를 1e12 magnitude로 자동감지) + epochSeconds/epochMillis 모두 반환; 날짜 문자열→epoch. 방향 자동감지(`/^-?\d+$/` 정수→epoch, 아니면 Date 파싱). createMuseTools 배선(22 도구).
- **왜:** 진안=개발자(Muse 빌드)는 로그/DB/API에서 epoch를 읽는데 12B가 주어진 epoch의 날짜를 fabricate(대수 날짜 산술 = 알려진 LLM 실패). 기존 time 도구 중 *주어진* epoch를 변환하는 건 없음(time_now=현재 instant, time_diff/add=ISO). Korean-utility vein 벗어난 fresh KIND·developer-grounding. 결정적이라 grounded-lie 0. value-class=EXPANSION(ratchet 권장 절반).
- **리뷰지점:** 알고리즘 node 선검증(JS Date authoritative) → RED(모듈 부재) → impl. **첫 컷 TS 에러(EpochResult 인터페이스가 JsonObject에 미할당 — 인덱스 시그니처 없음) → explicit 리터럴 반환으로 수정**(vitest-no-typecheck 교훈 재확인: 테스트 통과해도 build 필수). 14 파일 green. eval STABLE 3/3 ×6(양방향+time_now/time_diff carve+IrrelAcc). ④b Opus judge PASS 5/5 — #2(알고리즘 독립검증: 1718000000 sec/ms 동일 instant·0·음수·1600000000 epochSeconds/Millis 일관, sec/ms 경계 999999999999→33658년·1e12→2001 정확)·#1(배선 22)·#3(방향 자동감지 무모호, "20260614" 8자리=epoch 방어적)·#4(time 도구와 비중복, 실 LLM 약점) 무자비 검증. judge non-blocking note: 1e12 임계가 2001 이전 ms 타임스탬프 오분류(희귀, docstring scoped) → optional unit hint follow-up ◦.
- **리스크:** 없음 — 순수 read 도구, JS Date 결정적, bad input→error(throw 안 함), 기존 21 도구 무영향. LESSON: 새 도구 impl 후 반드시 build(인터페이스→JsonObject 미할당은 vitest가 못 잡고 tsc만 잡음). epoch sec/ms 자동감지는 본질적 휴리스틱(magnitude) — 실 timestamp(2001+) 정확, 경계는 명시 scoping. 다음: 다른 KIND/패키지.


## fire 135 · 2026-06-14 · skill v1.14.0 · f210b176
meta: value-class=correctness-bug(weather 상대-날짜 서버-tz vs 위치-tz 불일치) · pkg=@muse/mcp · kind=weather resolveForecastLine 위치-타임존 날짜 해석 · verdict=PASS · firesSinceDrill=5
ratchet: testFiles 1015 유지(weather-tool.test +isoInZone+cross-tz, weather-daily-forecast.test 시그니처 갱신) · fabrication 0 유지 · eval 무변동(선택/스키마 무변경) · 패키지 @muse/tools→@muse/mcp 다양화
- **무엇:** weather 도구가 상대 `when`("today"/"tomorrow")을 SERVER tz(localDateIso=getFullYear/Month/Date)로 해석 후, 위치 tz로 dated된 dailyForecast 날짜와 매칭 → KST 유저가 "LA 내일 날씨" 물으면 틀린 날(또는 false "no forecast") 반환(grounded-lie). 새 `isoInZone(instant, tz)`(ICU, machine-독립) + resolveForecastLine이 상대 타겟을 geocode 후 location.timezone로 해석. 명시 ISO는 tz-독립 유지. resolveForecastLine 시그니처 `ForecastTarget({iso}|{relative,now})` → `{date,line}`, dead resolveTargetDateIso/localDateIso 제거.
- **왜:** Muse=local-first 개인 AI, KO 유저가 해외 날씨 묻는 건 흔함(여행/지인). 서버-tz vs 위치-tz 불일치는 KST→US에서 자주 발생(14-17h 오프셋). grounding floor 정렬(틀린 날 confident answer 제거). 패키지 다양화(@muse/tools 4연속→mcp), fire-130과 다른 weather 버그-클래스(invalid-date vs cross-tz). scout가 발견한 top real gap(비-tools 표면은 얇음).
- **리뷰지점:** scout(Opus)가 cross-tz를 top priority-1로 발견. RED(isoInZone 부재 + cross-tz 케이스, machine-독립 위해 isoInZone unit test 결정적 + LA stub 통합) → impl(2파일+index+시그니처 변경) → 시그니처 변경이 weather-daily-forecast.test의 직접 호출 3건 노출 → {iso}로 갱신(+SEOUL tz로 'tomorrow' 테스트 machine-독립화). 195 mcp 파일 green·lint clean. ④b Opus judge PASS 5/5 — #2(isoInZone DST springforward/fallback/year-rollover 독립검증, en-CA formatToParts robust)·#1(서버-tz→위치-tz 행동 델타 RED 재현)·#3(fire-130 invalid-date 보존, ISO-miss date echo 보존)·#4(semantics: forecast 날짜=위치-local이라 상대도 위치-tz가 정답, UTC fallback sound)·#5(시그니처 변경 무broken-caller) 무자비 검증.
- **리스크:** 없음 — 순수 read, ICU 결정적, 명시 ISO 무shift, no-tz는 UTC fallback(open-meteo auto와 일관), 기존 동작 보존. LESSON: 서버-tz 의존 로직은 머신-종속 테스트를 만든다 → 명시 tz(Intl timeZone)로 결정적·정확. 시그니처 변경 시 직접 호출 테스트도 갱신(컴파일 통과해도 vitest 런타임 fail). geocode-ambiguity runner-up → backlog ◦. pnpm check agent-core FAIL은 동시 루프(무관). 다음: 다른 KIND/패키지.


## fire 136 · 2026-06-14 · skill v1.14.0 · 66086f60
meta: value-class=new-capability(EXPANSION) · pkg=@muse/tools(+scripts eval) · kind=number_base 진법 변환 도구(2/8/10/16) · verdict=PASS · firesSinceDrill=6
ratchet: testFiles 1016→1017(+muse-tools-number-base.test) · fabrication 0 유지 · eval:tools +1 시나리오(number-base 6 케이스 STABLE 3/3) · 새 KIND(radix); scout 후보 기각→pivot
- **무엇:** scout가 추천한 calendar all-day-date 버그를 적대 검증 → 컨벤션 모호(기존 테스트+caldav/google read가 UTC-midnight 가정 → UTC-slice 내부 일관, scout이 macOS 경로에 과적합) → DEFER(backlog ◦, 컨벤션 감사 선행 필요) → 검증가능한 `number_base`로 pivot. BigInt 정확(DEADBEEFCAFE1234가 parseInt에선 float-round = grounded-lie, BigInt로 정확), 0x/0b/0o+음수, from/to 4-value enum. createMuseTools 배선(23 도구).
- **왜:** 진안=개발자, 12B는 multi-digit radix 변환 틀림(dumps/bitmask/주소). 기존 도구 비중복(base64=byte 인코딩≠숫자 radix, math_eval=연산자, epoch=시간). EXPANSION value-class(ratchet 권장 절반), 새 KIND. **scout 후보를 무비판 수용 안 함**: 적대 검증으로 컨벤션 모호 발견 → 검증불가 변경 안 함(fail-closed), 깨끗한 슬라이스로 전환.
- **리뷰지점:** 알고리즘 node 선검증(BigInt=정확, parseInt=float-loss 대비) → RED(모듈 부재) → impl → 15 파일 green. eval STABLE 3/3 ×6(3방향+math_eval/unit_convert carve+IrrelAcc). ④b Opus judge PASS 5/5 — #2(알고리즘 독립검증: DEADBEEFCAFE1234→16045690984503054900 === BigInt("0x...")확인, parseInt는 ...054000으로 손실; 무효digit/unknown-base/empty→undefined, 80-f도 정확, silent-wrong 없음)·#1(배선 23)·#3(비중복 + BigInt-exactness 실 grounding win)·#4(enum 4-value가 raw base보다 12B-friendly) 무자비 검증. judge가 calendar defer를 "right call, fail-closed" 명시 승인.
- **리스크:** 없음 — 순수 read, BigInt 결정적·정확, bad input→error(throw 안 함), 기존 22 도구 무영향. LESSON: scout 추천도 적대 검증 — calendar all-day는 컨벤션 모호(UTC-midnight vs local-midnight)라 "버그"가 실은 일관 컨벤션일 수 있음 → 검증불가 변경 거부(uncertain⇒stop)하고 검증가능 슬라이스로 pivot이 정답. parseInt 2^53 초과 float-loss는 grounding-floor 제품에 grounded-lie → BigInt 필수. 다음: 다른 KIND/패키지(또는 calendar 컨벤션 감사를 큰 ◦로).


## fire 137 · 2026-06-14 · skill v1.14.0 · ac3079df
meta: value-class=correctness-bug(grounding-verdict hedge-then-assert fail-open) · pkg=@muse/recall · kind=groundingVerdictNotice pure-refusal 게이트 · verdict=PASS · firesSinceDrill=7
ratchet: testFiles 1018→1018(verdict.test+text.test 케이스 추가, 새 파일 없음) · fabrication 0 강화(fail-open 봉쇄) · eval 무변동(결정적 grounding, MuseTool 아님) · 패키지 @muse/tools→@muse/recall 다양화
- **무엇:** `groundingVerdictNotice`(채팅 recall의 하드 grounding verdict)가 `answerIsRefusal`(SUBSTRING)로 short-circuit → "I don't have access to flights, but your flight is at 9am"같은 hedge-then-assert가 refusal substring 매칭으로 통과 → 조작된 claim이 `grounded`로 라벨됨(fabrication fail-open). 새 `answerIsPureRefusal`(문장/역접 seam EN+KO로 split, non-refusal clause가 ≥2 토큰이면 false)로 하드 게이트만 교체; 14개 advisory 사이트는 lenient answerIsRefusal 유지.
- **왜:** Muse의 핵심 차별점=grounding floor(fabrication=0). substring refusal 체크가 hedge-then-assert에 fail-open이면 조작 claim이 grounded로 빠져나감 — 정확히 floor가 막아야 할 것. 패키지 다양화(@muse/tools converter 집중에서 벗어남), grounding-core KIND. 보수적 bias(모호하면 verdict 실행=over-warn, never block)가 fabrication=0에 안전한 방향.
- **리뷰지점:** scout(Opus)가 grounding-core에서 발견 → **나 자신이 적대 검증**(text.ts substring + verdict.ts:64 short-circuit 직접 확인, 컨벤션 모호 아닌 명확한 fail-open). RED(hedge-then-assert가 short-circuit) → answerIsPureRefusal impl + 게이트 교체 → recall 40파일+cli refusal/sufficiency 2파일 green·lint clean. ④b Opus judge(grounding-floor 최대 스크루티니) PASS 5/5 — #1(mutation-sensitive RED→GREEN)·#2(fail-open 실재+봉쇄, node 독립검증: hedge=false/pure=true/sentence-separated=false/KO 모두 정확; **잔여 hole 정직 식별**: em-dash/colon/bare-comma 조인트는 여전히 통과하나 OLD도 그랬으니 regression 아닌 strict-superset)·#3(불변식 무약화, pure refusal 여전히 short-circuit)·#4(14 advisory 사이트 무회귀)·#5(throw 없음, ≥2 임계 defensible) 무자비 검증.
- **리스크:** 낮음 — 보수적(over-warn, never block), pure refusal 보존, advisory 사이트 무영향. 잔여 hole(—/:/bare-comma split 미포함)은 backlog ◦(strict-superset이라 regression 아님, widening). LESSON: scout 추천도 적대 자가검증 — 이번엔 fail-open이 *명확*(calendar처럼 모호하지 않음)이라 진행; substring refusal 체크는 hedge-then-assert에 본질적 fail-open(grounding 게이트엔 clause-aware 필요). 다음 fire 138 = JUDGE-DRILL(연속 allPASS 8 도달). pnpm check agent-core FAIL은 동시 루프(무관).


## fire 138 · 2026-06-14 · skill v1.14.0 · f7b550c1
meta: value-class=new-capability(EXPANSION) + JUDGE-DRILL · pkg=@muse/tools(+scripts eval) · kind=leap_year 윤년 판정(Gregorian century exception) · verdict=PASS · firesSinceDrill=0(드릴 완료, 리셋)
ratchet: testFiles 1018→1019(+muse-tools-leap-year.test) · fabrication 0 유지 · eval:tools +1 시나리오(leap-year 6 케이스 STABLE 3/3) · 새 KIND(leap year)
- **무엇:** [JUDGE-DRILL] 연속 allPASS 8 도달 → 고의 나쁜-슬라이스 주입: naive `year % 4` 규칙(grounded-lie: 1900→leap, 2100→leap)의 leap_year 도구 + **틀린 값을 박은 테스트**(isLeapYear(1900)===true) → 결정적 게이트 통과(빌드/16파일/inventory 24/lint) → ④b Opus judge가 node로 dist 독립 검증해 **FAIL**(1900/2100/2200이 leap로 나옴=grounded-lie + 테스트가 버그를 lock-in함 적발) → 수정. [REAL] 전 Gregorian 규칙(`% 4 === 0 && (% 100 !== 0 || % 400 === 0)`)으로 fix, century exception 테스트로 regression-lock(1900/2100/2200→false, 2000/1600→true). createMuseTools 배선(24 도구).
- **왜:** [드릴] 연속 allPASS≥8 하드-카운터 — 새 실패 클래스(subtly-wrong-logic/grounded-lie 값, 기존 inert/cosmetic/coverage-theater와 다른 변종)로 judge의 "알고리즘 독립검증, 테스트 불신" 근육 재확인. [실수정] century exception은 12B가 실제로 틀리는 케이스(÷4는 맞지만 ÷100-not-÷400 틀림) → 결정적 grounding. 비중복(math_eval/time/number_base와 다름).
- **리뷰지점:** [드릴] 결정적 게이트 전부 통과하나 judge가 node 독립검증으로 1900/2100/2200 false여야 함을 적발 + 테스트가 틀린 값 인코딩함도 적발 → 롤백/수정. [실수정] 수정 알고리즘 node 독립검증(12 케이스 전부 정확) → 16파일 green. eval STABLE 3/3 ×6. ④b(fresh) PASS 5/5 — #2(Gregorian 독립검증 1900/2000/2100/2400 + 테스트가 정값 lock)·#1(배선 24)·#3(century exception=실 12B 실패, 비중복)·#5(2024.7→trunc lenient지만 defensible) 검증.
- **리스크:** 없음 — 순수 read, 결정적·정확(judge 독립검증), bad input→error. LESSON: JUDGE-DRILL 변종 로테이션(inert→cosmetic→coverage-theater→**subtly-wrong-logic**)으로 verifier discrimination 폭 확인; "테스트 통과 ≠ 정확"을 judge가 node 독립검증으로 일관 적용함 재확인(드릴 subject를 정확히 ship). 다음: 드릴 리셋 후 다른 KIND/패키지(@muse/tools 집중 완화). pnpm check agent-core FAIL은 동시 루프(무관).


## fire 139 · 2026-06-14 · skill v1.14.0 · (no slice — blocked)
meta: value-class=blocked(grounding-verdict 분리자 widening이 pure-refusal 회귀) · pkg=@muse/recall(시도, 롤백) · kind=answerIsPureRefusal CLAUSE_SPLIT_RE widening · verdict=NO-SLICE · firesSinceDrill=1
ratchet: testFiles 1019 유지(시도 롤백) · fabrication 0 유지(회귀 안 머지) · eval 무변동 · @muse/tools 5/8 다양화 시도(recall 택했으나 blocked)
- **무엇:** fire-137 follow-up ◦(hedge-then-assert 분리자 widening)를 시도 — CLAUSE_SPLIT_RE에 em-dash(—)+colon(:) 추가. TDD RED(em-dash/colon hedge가 pure로 새어나감) 확인했으나 GREEN 시 **3개 기존 pure-refusal 테스트 회귀**: "I'm not sure — that isn't in your notes."가 em-dash split로 "that isn't in your notes" clause 분리 → 이게 refusal marker("not in your notes")의 축약형("isn't")이라 매칭 안 됨 → 실질 주장으로 오분류 → pure=false → 정직한 refusal에 거짓 ⚠️ 경고. **롤백**(git restore, 회귀 안 머지).
- **왜:** 분리자 widening은 본질적으로 fragile — clause-level answerIsRefusal marker 검출의 빈틈을 노출(refusal 연속구가 marker 미매칭이면 hedge로 오판). whack-a-mole. 견고한 해법은 judge가 fire-137에 제안한 INVERT(refusal 후 content가 NEW factual assertion인지 의미/구조 판정), 분리자 추가 아님. >1 fire 또는 전용 설계 필요.
- **리뷰지점:** RED→GREEN 시도 중 3 회귀 테스트 발견(judge 호출 전 결정적 게이트가 잡음) → "불변식 약화/uncertain⇒stop"으로 롤백. 잔여 hole(em-dash/colon/bare-comma)은 regression 아님(구 substring도 통과). ④b judge 미호출(슬라이스 롤백). backlog ◦를 INVERT-필요 + BLOCKED로 갱신.
- **리스크:** 없음 — 회귀를 머지 안 함(grounding floor 보호). LESSON: heuristic 강화는 "더 많은 분리자"가 아니라 검출 견고성(의미 판정)이 축 — fragile 패치를 fail-close로 거부. 다양화 시도는 옳았으나(recall, @muse/tools 탈피) 선택 슬라이스가 blocked → 정직 종료. 다음: INVERT 접근 또는 다른 KIND/패키지(@muse/tools 집중 완화 계속). pnpm check agent-core FAIL은 동시 루프(무관).

...
## fire 16 · 2026-06-21 · skill v2.0.0 · a7b95355
meta: value-class=wiring · pkg=@muse/memory+@muse/recall · kind=freshness-mark-at-point-of-use · verdict=PASS · firesSinceDrill=6
ratchet: testFiles +0 (extended existing) · memory 484 pass · recall 373 pass · cli build clean · pnpm check exit0 (saturation timeouts on UNRELATED synthetic tests cleared on isolated rerun) · pnpm lint exit0 · fabrication 0 · self-eval green
- **DOCTRINE:** advances ③ consolidation spine sub-slice #3 — `classifyFactFreshness` (built, proven)
  was dead at the grounding surface; now wired so a months-stale auto-fact is cautioned, not asserted.
- **What:** new `staleFactKeys` (@muse/memory, reuses classifyFactFreshness — default staleDays=90,
  fail-soft) + `buildMemoryContextBlock` (@muse/recall) renders `STALE_FACT_MARK`
  ("(last confirmed a while ago — may be out of date)") with precedence contested→provisional→stale;
  commands-ask derives staleMemoryKeys from the SAME provenance already fetched (zero new I/O), fail-soft.
- **Why:** a fact confirmed ONCE long ago and never since (never flipped=not contested, seen=not
  provisional, just OLD) was asserted as confident truth — the doctrine's worst failure (confident wrong
  memory). SSGM (arXiv:2603.11768, temporal obsolescence/stale-address harm), mem0 (staleness named a
  hardest 2026 problem), arXiv:2603.07670. hermes/openclaw flag no recalled-memory staleness at point-of-use.
- **Review point:** precedence suppresses double-marks (contested/provisional key never also gets stale);
  90-day conservative bar + fail-soft (unparseable→fresh→no mark) bound over-caution.
- **Risk:** none to floor — additive mark (value still through defang/escape; set-equality + citation

...
2. **FIND — prioritized, never-empty.** Walk these in order; take the FIRST
   that yields a real item, then stop searching:
...
5. **VERIFY (fail-closed, maker≠judge).** A slice is NOT done until:
...
6. **SHIP** — one Conventional Commit (English subject), then **`git push`** the
   current branch (standing authorization above) — ONLY after VERIFY is green.
   Then WRITE-BACK: update the backlog (mark done, refill any findings). A loop
   re-invokes for the next slice; an interactive caller gets a short Korean
   report.

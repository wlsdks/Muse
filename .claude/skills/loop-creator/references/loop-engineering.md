...
> **v3.0.0+ 운영 주의**: 운영 골격(fire-프롬프트·등록 파이프라인)은 SKILL.md가
> **단일 소스**다. 이 문서는 원칙·근거·출처(왜 그 가드인가)를 담는다 — 두 문서의
> 골격 문구가 다르면 SKILL.md가 이긴다.
...
## §7 — 2026-07-18 재조사 (v3.0.0의 근거)

진안 지시로 2026-07-17 기준 실무자·기업·연구를 재조사(haiku 2방향 스윕 → fable
독립 검증). **검증 통과한 원전만** 여기 산다; 서브에이전트가 가져온 나머지 주장
(예: "반복 3-4회 정체 수치", "LoopTrap" 명명, Meta 60T 토큰 일화, 애그리게이터
재탕 기사들)은 원전 미확인으로 **unverified — 인용 금지**.

### 검증된 원전 (블로그·문서)

- **Addy Osmani, "Loop Engineering" (2026-06-07, addyosmani.com)** — 루프 5요소
  (Automations·Worktrees·Skills·Plugins/MCP·Sub-agents) + **6번째=외부 영속
  메모리**(markdown/보드가 컨텍스트 한계를 보상) + "무감독 자동화는 미탐지 실수를
  만든다"(검증·이해는 사람 몫으로 남는다).
- **Geoffrey Huntley, Ralph 루프 (2026-01-17, ghuntley.com/loop)** — fresh context
  per iteration, 디스크가 메모리, **모놀리식: 루프당 정확히 1태스크**, 사람은
  "루프를 관찰"하며 실패 도메인을 좁힌다. (우리 fire당-1슬라이스 캡의 외부 근거.)
- **Anthropic Claude Code best practices (code.claude.com, 2026)** — 결정론
  verifier(테스트/빌드/스크립트)가 정지 기준, maker≠judge fresh-context 리뷰,
  Explore→Plan→Implement→Verify 단계 분리.

### 검증된 공개 연구

- **arXiv 2606.19544 "Reliability without Validity" (2026-06)** — LLM 판정자는
  자기일관성(신뢰도 >0.95)이 높아도 타당성과 별개(심한 위치 편향 공존; exact-match
  일치는 우연 미보정으로 변별력을 과대평가, MT-Bench에서 카파 수축 33-41pp).
  → JUDGE-DRILL(주기적 fault-injection)의 직접 근거.
- **arXiv 2607.07663 (2026-07, 자기개선 서베이 1,250편)** — 실패모드로
  self-confirming loop·model collapse·diversity collapse를 분류. → 다양성
  래칫·독립 judge의 방향 근거(수치 주장은 이 서베이 초록에 없음 — 방향만 채택).
- **arXiv 2510.18254** (기존 검증 유지) — 외부 검증 없는 bare-retry는 85.36%
  같은 실패 반복. → retry ≤3 + 검증자-필수.

### v3.0.0에서 제거/축소한 하네스 (진안 승인 "과한 건 제거")

- 논문-scout 우선 블록 → ② 한 줄(연구-주도 항목 조건 + 수치 독립검증 의무)로 축소
  — 실사용 빈도 대비 fire당 토큰 비용 과다.
- DECOMPOSE-ON-DEFER·EXHAUSTION·다양성 RATCHET 3중 서술 → "선택 규율" 단일 블록
  (같은 판단 축의 중복 서술이었음; 가드 자체는 전부 유지).
- 모델 티어링 이중 서술 → 1곳. value-class 서사 → meta 카운트만.
- §4의 `/loop` 스킬 의존 → `CronCreate` 직접(존재하지 않는 스킬 참조는 문서 버그).
- 신규 가드: no-progress 브레이커(같은 실패 시그니처 2회=접근 전환), LLM-단독
  완료판정 금지 명문화, fresh-context(Ralph) 명시, 오너-강조 우선(v2.1.1 승계).

...
도구 [tool-design](tool-design.md)·[skills-and-mcp](skills-and-mcp.md)(규약) · **권한** [permission-matrix]
(permission-matrix.md)+`permissionGate` 코드 ✅ · **훅** [hooks](hooks.md)+`hooks.mjs` 코드 ✅(PreToolUse
...
`session.mjs`)도 코드 ✅ — 멈춘 실행을 완료 단계 재실행 없이 재개. 남은 정통 요소: 메모리 런타임 컴포넌트.
